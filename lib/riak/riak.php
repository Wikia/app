...
class RiakClient {
  ...
  /**
   * Set proxy server for this RiakClient.
   */
  ...
  function isAlive() {
    ...
    $response = RiakUtils::httpRequest('GET', $url, array(), '', $this->proxy);
...
class RiakMapReduce {
  ...
  function run($timeout=NULL) {
    ...
    $response = RiakUtils::httpRequest('POST', $url, array(), $content, $this->client->getProxy());
...
class RiakObject {
  ...
  function delete($dw=NULL) {
    ...
    # Run the operation...
    $response = RiakUtils::httpRequest('DELETE', $url, array(), '', $this->client->getProxy());
...
class RiakUtils {
   ...
   */
  public static function httpRequest($method, $url, $request_headers = array(), $obj = '', $proxy = false) {
    # Set up curl
    $ch = curl_init();
