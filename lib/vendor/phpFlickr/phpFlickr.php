...
if ( !class_exists('phpFlickr') ) {
	...
	class phpFlickr {
		...
		function post ($data, $type = null) {
			...
			if ( function_exists('curl_init') ) {
				...
				if ($this->proxy) {
					...
					// don't set "Expect: 100-continue" request header - it confuses Squid
					// @see http://www.iandennismiller.com/blog/2009/09/curl-http1-1-100-continue-and-multipartform-data-post/
		...
		function setProxy ($server, $port) {
			...
			// Wikia change - begin
			$this->proxy = array('server' => $server, 'port' => intval($port));
			// Wikia change - emd
		}
