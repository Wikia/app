describe('Image Serving', function () {
	...
	using('image sizes', [
		...
		it('correctly cuts image', function() {
			...
			expect(cutOptions.xOffset1).toBe(image.xOffset1);
			expect(cutOptions.yOffset1).toBe(image.yOffset1);
			expect(cutOptions.xOffset2).toBe(image.xOffset2);
			expect(cutOptions.yOffset2).toBe(image.yOffset2);
		});
	});
});
