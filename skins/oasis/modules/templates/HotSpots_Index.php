...
<?php if ( count( $data ) == 5 ): ?>
	<h2><?= wfMessage( 'myhome-hot-spots-newest' )->escaped(); ?></h2>
	<ul>
	<?php foreach ( $data as $item ): ?>
		<li>
			<span><a href="<?= Sanitizer::encodeAttribute( $item['url'] ); ?>" class="title" rel="nofollow"><?= htmlspecialchars( $item['title'] ); ?></a></span>
		</li>
	<?php endforeach; ?>
	</ul>
<?php elseif ( count( $data ) == 2 ): ?>
	<h2><?= wfMessage( 'myhome-hot-spots-feed' )->escaped(); ?></h2>
	<p><?= wfMessage( 'myhome-hot-spots-definition', $data['interval'] )->escaped(); ?></p>
	<?php
	...
	<?php foreach ( $data['results'] as $row ): ?>
		...
		<li<?= $hotSpotFire ?>>
			...
			<span><a href="<?= Sanitizer::encodeAttribute( $row['url'] ); ?>" class="title" rel="nofollow"><?= htmlspecialchars( $row['title'] ); ?></a></span>
		</li>
<?php
...
<?php else: ?>
	<?= wfMessage( 'myhome-hot-spots-brandnew' )->escaped(); ?>
<?php endif; ?>
</section>
