...
class UserPagesHeaderModule extends Module {
	...
	public function executeIndex() {
		wfProfileIn(__METHOD__);
		global $wgTitle, $wgEnableUserProfilePagesExt, $wgRequest, $wgUser;
		
		global $wgOut;
		$wgOut->redirect("http://bernhard.wikia-dev.com/wiki/User:OwenDavis");
		...
		if ($wgEnableFacebookSync == true && $wgUser->isLoggedIn() && self::isItMe( $this->userName ) && isset($this->content_actions['edit']) && $this->isUserProfilePageExt && $namespace == NS_USER) {
			...
			// Facebook sync info
			define('FACEBOOK_APP_ID', '115147731894314'); // needs to change to wikias app id in the future
			define('FACEBOOK_SECRET', 'a0749dd82bbd37448e6e5a3c5d77f0ea');
			...
			// sync button
			$fbAccessRequestURL = "https://graph.facebook.com/oauth/authorize?client_id=".FACEBOOK_APP_ID."&scope=".$facebookScope."&redirect_uri=".FACEBOOK_REDIRECT_URL;
	...
	public function executeFacebookConnect($arg) {
		...
		if ($arg['fbAccess'] == true) {
			...
			$facebook = new FacebookAPI(array('appId' =>FACEBOOK_APP_ID,'secret'=>FACEBOOK_SECRET,	'cookie' =>true, ));
			
			$token_url = 'https://graph.facebook.com/oauth/access_token?client_id=' .FACEBOOK_APP_ID .'&redirect_uri=' .FACEBOOK_REDIRECT_URL .'&client_secret=' .FACEBOOK_SECRET .'&code=' .$wgRequest->getVal( 'code' );
