...
<?php		wfProfileIn( __METHOD__ . '-article'); ?>
			<div id="article" <?php if($this->data['body_ondblclick']) { ?>ondblclick="<?php $this->text('body_ondblclick') ?>"<?php } ?>>
				...
				<div id="bodyContent">
					...
					*/
					if ($wgTitle->getPrefixedDBKey() == 'Special:Search') {
					?>
	...
	<?
	$afs_keywords = '';
	$afs_object = Revision::newFromTitle(Title::newFromText('MediaWiki:AFS'));
	if (is_object($afs_object)) {
		$afs_string = str_replace(' ', '', trim( $afs_object->getText() ) );
		$afs_array = explode(',', $afs_string);
	...
	?>
      ...
      google_afs_query = GetParam('search') + '<?=$afs_keywords?>';
      google_afs_ad = 'w2n8'; // specify the number of ads you are requesting
      //sample client and channel 
...
/*
			$christiantest = Revision::newFromTitle(Title::newFromText('User:BladeBronson/test'));
		if(is_object($christiantest)) {
			$christiantext = $christiantest->getText();
			echo $christiantext;	
			}
			*/
