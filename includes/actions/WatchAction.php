...
class WatchAction extends FormAction {
	...
	public static function doUnwatch( Title $title, User $user  ) {
		...
		if ( Hooks::run( 'UnwatchArticle', array( &$user, &$page ) ) ) {
	...
	public function onSuccess() {
		# start wikia change, mediawiki merge 1.19 by eloy
		global $wgEnableWikiaFollowedPages;
		if ( $wgEnableWikiaFollowedPages ) {
			$this->getOutput()->redirect( $this->getTitle()->getFullUrl( "action=view" ) );
		}
		# end wikia change
