...
class Title {
	...
	private function checkSpecialsAndNSPermissions( $action, $user, $errors, $doExpensiveQueries, $short ) {
		...
		$condition =  ( $this->getText() != 'Createpage' ) && ( $action != 'create' );
		if ( NS_SPECIAL == $this->mNamespace && !in_array( $action, $specialOKActions ) && $condition ) {
	...
	/**
	 * Check if upload of file is enabled
	 *
	 * @author Krzysztof Krzyżaniak (eloy) based on previous patch from Uberfuzzy
	 *
	 * @param $action String the action to check
	 * @param $user User user to check
	 * @param $errors Array list of current errors
	...
	private function checkUserBlock( $action, $user, $errors, $doExpensiveQueries, $short ) {
		...
		if ( ( $action == 'edit' || $action == 'create' ) && !$user->isBlockedFrom( $this, true ) ) {
	...
	private function checkReadPermissions( $action, $user, $errors, $doExpensiveQueries, $short ) {
		...
		if ( $useShortcut ) {
			...
			|| $this->isSpecial( 'Signup' )  # added by wikia
			|| $this->isSpecial( 'Captcha' ) # added by wikia
		) {
	...
	protected function getUserPermissionsErrorsInternal( $action, $user, $doExpensiveQueries = true, $short = false ) {
		...
		if ( $action == 'read' ) {
			...
			$checks = array(
				...
				'checkUploadDisabledRestrictions', # added by wikia
				'checkUserBlock'
			);
		}
