...
CONTROL;
		...
		if ( ! $this->loadRevisionData() ) {
			//displaying messages with empty user talk page - Marooned[at]wikia.com
			$msg_temp = false;
			...
			if ( $msg_temp !== false ) {
				...
				$mtext = $msg_temp;
				$this->mOldid = false;
				//if talk page doesn't exist, create a fake revision with messages as a content
				//$this->mNewRev = new Revision(array('text' => $msg_temp));
				//nope - create an empty revision in order not to display messages twice [those are displayed from loadText()/loadNewText()]
	...
	function loadText() {
		...
		if ( $this->mNewRev ) {
			...
			$msg_temp = false;
			wfRunHooks( 'GetUserMessagesDiffCurrent', array( $this->mTitle, &$msg_temp ) );
			$this->mNewtext = $msg_temp != false ? $msg_temp . $this->mNewtext : $this->mNewtext;
