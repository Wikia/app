...
//		parent::__construct( $params );
	...
	public function getMulti( $keys ) {
		...
		$this->debug( "getMulti for " . count( $keys ) . " keys " . "returned " . count( $result ) .
					  " results" );
	...
	public function add( $key, $value, $expiry = 0 ) {
		...
		try {
			if ( $expiry ) {
				$result = $conn->set( $key, $this->serialize( $value ), [ 'nx', 'ex' => $expiry ] );
			} else {
	...
	protected function getConnection( $key ) {
		...
		while ( ( $tag = array_shift( $candidates ) ) !== null ) {
			...
			$conn = $this->redisPool->getConnection( $server, null );
			if ( !$conn ) {
				continue;
			}
	...
	protected function getMasterLinkStatus( RedisConnRef $conn ) {
		...
		return isset( $info['master_link_status'] ) ? $info['master_link_status'] : null;
	}
...
//		$this->logger->error( "Redis error: $msg" );
	...
	public function logRequest( $method, $key, $server, $result ) {
		$this->debug( "$method $key on $server: " . ( $result === false ? "failure" : "success" ) );
	}
}
