<?php
abstract class FormInputField extends FormElement {
	...
	public function __construct($options = []) {
		...
		if (isset($options['template'])) {
			$this->setProperty(self::PROPERTY_TEMPLATE, $options['template']);
		}
	...
	public function render($htmlAttributes = [], $index = null) {
		return $this->renderInternal( $this->getProperty(self::PROPERTY_TEMPLATE ), $htmlAttributes, [], $index );
	}
	 ...
	 *
	 * @param string $templateName template type name
	 * @param array $htmlAttributes html attributes for field tag
	 ...
	 * @return string
	 */
	protected function renderInternal($templateName, $htmlAttributes = [], $data = [], $index = null) {
		$out = '';
		...
		$out .= $this->renderView($templateName, 'render', $data);
	...
	protected function renderErrorMessage() {
		...
		if (!empty($errorMessage)) {
			return $this->renderView($this->getProperty(self::PROPERTY_TEMPLATE), 'errorMessage', ['errorMessage' => $errorMessage]);
		}
	}
}