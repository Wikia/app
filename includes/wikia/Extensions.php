...
if ( $wgEnableLeftNavWidget === null && in_array( $wgLanguageCode, $wgAvailableLetNavWidgetLang ) ) {
	$wgEnableLeftNavWidget = true;
}
...
if (!empty($wgEnableOggHandlerExt)) {
	...
	/* From OggHandler/OggHandler.php: */
	//
	...
	//
	$wgCortadoJarFile = 'http://images.wikia.com/cortado-ovt-stripped-wm_r36976.jar';
}
...
/**
 * Maps few categories to English lifestyle wikias.
 *
 * These mapping must be done to all future wikias in this category too,
 * hence it is done here as opposed to a one-time script
 *
 * @see VID-1811 & VID-1830
 */
if ( $wgEnableVideosModuleExt !== false ) {
	// db names of wikias to be excluded
	$excludeVMWikis = [
		'cookbook_import',
		'healthyrecipes',
		'fashion',
	...
	$catIncludeVMWikis = [
						'Food and Drink' => [ 'Celebrations', 'Food' ],
						'Lifestyle' => [ 'Celebrations', 'Family', 'Living' ],
						'Technology' => [ 'Tech' ],
	];

	if ( $wgLanguageCode == 'en'
	     && !empty( $wgHub )
	     && isset( $catIncludeVMWikis[$wgHub->cat_name] )
	     && !in_array( $wgDBname, $excludeVMWikis ) ) {

		$wgEnableVideosModuleExt = true;
		$wgVideosModuleCategories = array_unique( array_merge( $wgVideosModuleCategories, $catIncludeVMWikis[$wgHub->cat_name] ) );
	}
}

if ( !empty( $wgEnableVideosModuleExt ) ) {
	include( "$IP/extensions/wikia/VideosModule/VideosModule.setup.php" );
}
...
if( !empty( $wgEnableVideoToolExt ) ||
    !empty( $wgEnableSpecialVideosExt ) ||
    !empty( $wgEnableVideosModuleExt ) ) {
...
$wgEnableSpotlightsV2_GlobalNav = true;
$wgEnableSpotlightsV2_Footer = true;
$wgEnableSpotlightsV2_Rail = true;
