...
abstract class IndexPager implements Pager {
	...
	function getPagingQueries() {
		...
		if ( isset($this->mShowAll) && ($this->mShowAll == true)){#ticet 699
		    $showAll = 1;    
		}else{
		        $showAll = 0;
		}
		...
		if ( $this->mIsFirst ) {
			...
			$prev = array( 'dir' => 'prev', 'offset' => $this->mFirstShown, 'limit' => $urlLimit, 'showall' => $showAll );
			$first = array( 'limit' => $urlLimit, 'showall' => $showAll );
		}
		if ( $this->mIsLast ) {
			$next = false;
			$last = false;
		} else {
			$next = array( 'offset' => $this->mLastShown, 'limit' => $urlLimit, 'showall' => $showAll );
			$last = array( 'dir' => 'prev', 'limit' => $urlLimit, 'showall' => $showAll );
		}
		...
		foreach ( $fields as $field => $name ) {
			if ( strval( $name ) == '' ) {
				...
				if ( $field == $this->mSort ) {
					...
					$s .= "<th class=\"$sortClass$sortSubClass\">$link</th>\n";
				} else {
