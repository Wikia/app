	...
	public static function dynamicPageList( $input, $params, $parser, &$bReset, $calledInMode ) {
		...
		foreach ( $aParams as $iParam => $sParam ) {
			...
			switch ( $sType ) {
				...
				case 'category':
					...
					foreach ( $aParams as $sParam ) {
						...
						if ( $sParam == '' ) { // ignore empty line
							...
							if ( $sParam == '_none_' ) { // include uncategorized pages (special value: empty string)
								$aParams[ $sParam ] = '';
								$bIncludeUncat = true;
								$aCategories[] = '';
							} else {
				...
				case 'mode':
					if ( in_array( $sArg, ExtDynamicPageList::$options[ 'mode' ] ) ) //'none' mode is implemented as a specific submode of 'inline' with <br/> as inline text
					{
						if ( $sArg == 'none' ) {
		...
		// SELECT ... FROM
		if ( $acceptOpenReferences ) // SELECT ... FROM
		{
			if ( count( $aImageContainer ) > 0 ) {
...
*/
						...
						if ( $suit == '♦' ) {
							...
							if ( $suit == '♥' ) {
								...
								if ( $suit == '♠' ) {
									...
									if ( $suit == 'sa' || $suit == 'SA' || $suit == 'nt' || $suit == 'NT' ) {
										$newkey .= '5 ';
									} else {
										$newkey .= $suit;
									}
	...
	// SUS-807
	private static function getWhereStatementForUsername( $username, $dbr ) {
		$res = [];
