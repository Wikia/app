...
class WallRelatedPages extends WikiaModel {
	...
	public function getArticlesRelatedMessgesSnippet( $pageId, $messageCount, $replyCount ) {
		...
		foreach ( $messages as $value ) {
			...
			$row['messageBody'] = $helper->getMessageSnippet( $wallMessage->getRawText(), $wallMessage->getTitle() );
			...
			foreach ( $replies as $reply ) {
				...
				if ( !$reply->isRemove() && !$reply->isAdminDelete() ) {
					$replyRow = [
						...
						'messageBody' => $helper->getMessageSnippet( $reply->getRawText(), $reply->getTitle() ),
