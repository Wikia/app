...
<li class="notifications-header">
	<span>
		<? if ( $wikiCount > 1): ?>
			<?= wfMessage( 'wall-notifications-all' )->escaped(); ?>
		<? else: ?>
			<?= wfMessage( 'wall-notifications' )->escaped(); ?>
		<? endif; ?>
		<? if( !empty($count) ): ?>
			<? if( $wikiCount == 1 || $notificationCounts[0]['unread'] == 0): ?>
				<div id="wall-notifications-markasread">
					<span id="wall-notifications-markasread-this-wiki" style="display: inline-block"><?= wfMessage( 'wall-notifications-markasread' )->escaped(); ?></span>
				</div>
			<? else: ?>
				<div id="wall-notifications-markasread">
					<span id="wall-notifications-markasread-sub"><?= wfMessage( 'wall-notifications-markasread' )->escaped(); ?></span>
					<div id="wall-notifications-markasread-sub-opts">
						<span id="wall-notifications-markasread-this-wiki"><?= wfMessage( 'wall-notifications-markasread-this-wiki' )->escaped(); ?></span>
						<span id="wall-notifications-markasread-all-wikis"><?= wfMessage( 'wall-notifications-markasread-all-wikis' )->escaped(); ?></span>
					</div>
				</div>
			<? endif; ?>
		<? endif; ?>
	...
	<? if (!empty($wikiData['sitename'])): ?>
		...
		<? endif; ?>
			...
			<ul class="notifications-for-wiki-list">
				<li class="notifications-empty"><?= wfMessage( 'wall-notifications-loading' )->escaped(); ?></li>
			</ul>
		</li>
	<? endif ?>
<? endforeach; ?>
