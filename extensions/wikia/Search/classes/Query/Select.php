...
use Wikia\Search\MediaWikiService, Sanitizer, Solarium_Query_Helper;
...
class Select {
	...
	public function getSanitizedQuery() {
		if ( $this->sanitizedQuery == null ) {
			$this->sanitizedQuery =
				html_entity_decode( Sanitizer::stripAllTags( $this->rawQuery ), \ENT_COMPAT, 'UTF-8' );
		}
	 ...
	 *
	 * @TODO applying word limit here is clumsy at best and suggests it's about time we implement some class
	 *     abstraction
	 * and hierarchy around what kinds of queries we're using where. this would be good for an actual edismax query,
	 *     not so great for video suggest or direct solr queries, for instance.
	 *
