...
class CombinedSearchService {
	...
	public function searchForArticles( $query, $namespaces, $wikias, $maxArticlesPerWiki ) {
		...
		foreach ( $wikias as $wiki ) {
			$currentResults =
				$this->querySolrForArticles( $query, $namespaces, $maxArticlesPerWiki, $wiki['wikiId'], $wiki['lang'] );
	...
	public function phraseSearchForArticles( $query, $namespaces, $langs, $hubs = null, $minArticleQuality = null ) {
		...
		foreach ( $langs as $lang ) {
			$currentResults =
				$this->queryPhraseSolrForArticles( $query, $namespaces, $lang, $hubs, $minArticleQuality );
			foreach ( $currentResults as $article ) {
	 ...
	 * @return array
	 */
	protected function queryPhraseSolrForArticles(
		$query,
		$namespaces,
		$lang,
		$hubs = null,
		...
		$requestedFields = [
			'title' => Utilities::field( 'title', $lang ),
			"url",
			"id",
			"score",
			"pageid",
		...
		$select->createFilterQuery( 'banned' )->setQuery(
			'-(wid:547090) AND -(wid:95) AND -(wid:43339) AND -(host:*answers.wikia.com)'
		);
		if ( $minArticleQuality != null ) {
			$select->createFilterQuery( 'article_quality' )->setQuery(
				'article_quality_i:[' . $minArticleQuality . ' TO *]'
			);
		}
	...
	public function queryPhraseForWikias( $query, $hubs, $lang ) {
		...
		//speedydeletion: 547090, scratchpad: 95, lyrics:43339,
		$select->createFilterQuery( 'banned' )->setQuery(
			'-(id:547090) AND -(id:95) AND -(id:43339) AND -(hostname_s:*answers.wikia.com)'
		);
	...
	public function queryForWikias( $query, $hubs, $lang ) {
		...
		$crossWikiSearchConfig->setQuery( $query )->setLimit( self::CROSS_WIKI_RESULTS )->setPage( 1 )->setRank(
				'default'
			)->setInterWiki( true )->setCommercialUse( $this->getHideNonCommercialContent() )->setLanguageCode( $lang );
		if ( !empty( $hubs ) ) {
			$crossWikiSearchConfig->setHubs( $hubs );
		}
		$crossWikiResultSet = ( new Factory )->getFromConfig( $crossWikiSearchConfig )->search();
		$crossWikiResults = $crossWikiResultSet->toArray(
			[ "sitename_txt", "url", "id", "description_txt", "lang_s", "score", "description_txt" ]
		);
...
//		$images = $this->wikiService->getWikiImages( $outputModel['wikiId'], self::IMAGE_SIZE, self::IMAGE_SIZE );
	...
	private function processArticle( $articleInfo ) {
		...
		if ( isset( $articleInfo['article_type_s'] ) ) {
			...
			$outputModel['type'] =
				array_key_exists( $type, $this->article_types_passed_on ) ? $type : static::ARTICLE_TYPE_OTHER;
		} else {
			...
			if ( $row ) {
				...
				if ( !empty( $dbName ) ) {
					...
					$imageServing = new \ImageServing(
						[ $articleId ], self::IMAGE_SIZE, self::IMAGE_SIZE, $db
					);
	...
	protected function getTopArticles( $wikiId, $lang ) {
		return \WikiaDataAccess::cache(
			wfSharedMemcKey( "CombinedSearchService", $wikiId, $lang ),
			self::TOP_ARTICLES_CACHE_TIME,
			function () use ( $wikiId, $lang ) {
				...
				$searchConfig = new Config;
				$searchConfig->setLimit( self::TOP_ARTICLES_PER_WIKI )
					->setQuery( $query )
					->setPage( 1 )
					->setRequestedFields( $requestedFields )
