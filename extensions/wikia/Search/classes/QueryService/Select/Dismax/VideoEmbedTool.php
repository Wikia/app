...
class VideoEmbedTool extends Video {
	...
	protected function getQuery() {
		return sprintf(
			'+(%s) AND ( +(%s)^1000 AND +(%s)^2000 )',
			$this->getQueryClausesString(),
			$this->getTopicsAsQuery(),
			$this->getTransformedQuery()
		);
	}

	/**
	 * I've noticed that first names or characters are more common in videos, and last names much less common
	 * This boosts the first token in a query, which often corresponds to a first name. We will probably want to tweak
	 * this.
	 */
