...
class InterWiki extends AbstractDismax {
	...
	protected function registerFilterQueryForMatch() {
		...
		if ( $config->hasWikiMatch() ) {
			$noPtt = Utilities::valueForField( 'id', $config->getWikiMatch()->getId(), [ 'negate' => true ] );
	...
	protected function getFilterQueryString() {
		...
		$filterQueries = [
			// SUS-5681 | allow wikis with promoted_wiki_b fields set to override articles threshold
			'( articles_i:[' . $this->config->getXwikiArticleThreshold() . ' TO *] OR promoted_wiki_b:true )',
			"-id:{$wid}",
			// PLATFORM-3659: Exclude hidden wikis from global search
			'-hidden_wiki_b:true'
		];
	...
	protected function getQueryClausesString() {
		...
		foreach ( $excludedWikiIds as $excludedWikiId ) {
			$widQueries[] = Utilities::valueForField( 'wid', $excludedWikiId, [ 'negate' => true ] );
		}
		$queryClauses = [];
		$config = $this->getConfig();
