...
abstract class AbstractDismax extends AbstractSelect {
	...
	protected function registerComponents( Solarium_Query_Select $select ) {
		return $this->registerDismax( $select )->registerNonDismaxComponents( $select );
	}
	...
	/**
	 * As an edismax query, gives the required query in the first clause of the conjunction, and then the parseable
	 * query stuff in the second clause.
	 *
	 * @return string
	 */
	protected function getQuery() {
		...
		return sprintf(
			'+%s AND +(%s)',
			$queryClauses,
			$this->getConfig()->getQuery()->getSolrQuery( self::MAX_QUERY_WORDS )
		);
	}
