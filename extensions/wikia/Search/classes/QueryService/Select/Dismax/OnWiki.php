...
class OnWiki extends AbstractDismax {
	...
	/**
	 * Registers different components in Solarium. We also use this spot to update query fields for the video search
	 * child class.
	 *
	 * @param \Solarium_Query_Select $query
	 */
	protected function registerNonDismaxComponents( Select $query ) {
		return $this->registerHighlighting( $query )->registerFilterQueries( $query )->registerSpellcheck( $query );
	}
	...
	protected function registerFilterQueryForMatch() {
		...
		if ( $config->hasArticleMatch() ) {
			$noPtt = Utilities::valueForField(
				'id',
				$config->getArticleMatch()->getResult()->getVar( 'id' ),
				[ 'negate' => true ]
			);
	...
	protected function getQueryClausesString() {
		$queryClauses = [ Utilities::valueForField( 'wid', $this->config->getCityId() ) ];
