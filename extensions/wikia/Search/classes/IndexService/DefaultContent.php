...
class DefaultContent extends AbstractService {
	...
	public function execute() {
		...
		$this->pushNolangTxt( $titleStr )->pushNolangTxt( preg_replace( '/([[:punct:]])/', ' $1 ', $titleStr ) );
	...
	protected function prepValuesFromHtml( $html ) {
		...
		$paragraphString =
			trim( preg_replace( static::SPACE_SEQUENCE_REGEXP, ' ', implode( ' ', $paragraphs ) ) ); // can be empty
				...
				if ( $infoboxRows ) {
					foreach ( $infoboxRows as $row ) {
						...
						if ( $infoBoxCellCount == 2 && $headerCellCount == 0 ) {
							$result['infoboxes_txt'][] = "infobox_{$counter} | " .
								preg_replace(
									'/\s+/',
									' ',
									$infoboxCells[0]->plaintext . ' | ' . $infoboxCells[1]->plaintext
								);
							...
							$result['infoboxes_txt'][] = "infobox_{$counter} | " .
								preg_replace(
									'/\s+/',
									' ',
									$headerCells[0]->plaintext . ' | ' . $infoboxCells[0]->plaintext
								);
						}
					}
				}
