...
class Config {
	...
	/**
	 * Set true if we need to apply some special treatment for commercial clients i.e. filter wikis with non-commercial
	 * license
	 *
	 * @var bool
	 */
	 ...
	 * If a query with a namespace prefix has been set and is not included in the namespaces, we also include this
	 * value in the namespace array.
	 *
	 * @return array
	 */
	...
	protected function articleMatchPassesFilters( \Wikia\Search\Match\Article $match ) {
		...
		return !( ( $minArticleQuality && ( $result['article_quality_i'] < $minArticleQuality ) ) ||
			( // We have a file that is video, but we only want images.
				$result['ns'] == NS_FILE &&
				in_array( Config::FILTER_IMAGE, $filterKeys ) &&
				$isVideoFile ) ||
			( // We have a file that is not a video, but we only want videos.
				$result['ns'] == NS_FILE &&
				in_array( Config::FILTER_VIDEO, $filterKeys ) &&
				!$isVideoFile ) );
	}
	...
	public function getSearchProfiles() {
		...
		$profiles = [
			...
			SEARCH_PROFILE_ADVANCED => [
				...
				'namespaces' => $this->getNamespaces(),
				'parameters' => [ 'advanced' => 1 ],
			]
		];

		$this->getService()->invokeHook( 'SpecialSearchProfiles', [ &$profiles ] );
	...
	public function setFilterQueries( array $filterQueries ) {
		...
		foreach ( $filterQueries as $filterQuery ) {
			if ( is_array( $filterQuery ) && isset( $filterQuery['query'] ) ) {
				$this->setFilterQuery(
					$filterQuery['query'],
					( isset( $filterQuery['key'] ) ? $filterQuery['key'] : null )
				);
