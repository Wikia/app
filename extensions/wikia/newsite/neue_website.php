...
function get_wikitext($dbw, $site, $domsite)
{
	...
	$contentsFile = NeueWebsiteHelper::getContentsFile($site);
	$logFile = NeueWebsiteHelper::getLogFile($site);
	$errorFile = NeueWebsiteHelper::getErrorFile($site);

	if(file_exists($contentsFile))
		$hfp = fopen($contentsFile, "r");
	...
	if($hfp && $lfp)
	{
		...
		if(filesize($contentsFile))
			$htcontent = fread($hfp, filesize($contentsFile));
		else
			$empty = true;
		$locontent = fread($lfp, filesize($logFile));
