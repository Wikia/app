...
Manager.prototype.onClose = function (id) {
  ...
  this.closed[id] = [];
  this.closed[id].issued = Date.now();
...
Manager.prototype.garbageCollection = function () {
  ...
  // clean up the handshakes
  while (i--) {
    data = this.handshaken[ids[i]];

    if ('issued' in data && (now - data.issued) >= expire) {
      this.onDisconnect(ids[i]);
    }
  }

  ids = Object.keys(this.closed);
  i = ids.length;
  expire = this.get('close timeout') * 1000;

  // clean up the closed sockets
  while (i--) {
    data = this.closed[ids[i]];

    if ('issued' in data && (now - data.issued) >= expire) {
      delete this.closed[ids[i]];
    }
  }
};
