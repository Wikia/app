...
class UserProfilePageController extends WikiaController {
	...
	public function renderLightbox() {
		...
		$tabs = array(
			array( 'id' => 'avatar', 'name' => wfMessage( 'user-identity-box-avatar' )->escaped() ),
			array( 'id' => 'about', 'name' => wfMessage('user-identity-box-about-me')->escaped() ),
		...
		if ( $isAllowed && isset( $data->source ) && isset( $data->file ) ) {
			switch ( $data->source ) {
				...
				default:
					...
					wfMessage( 'userprofilepage-interview-save-internal-error' )->escaped();
					break;
			}
	...
	private function renderAvatarLightbox( $userId ) {
		...
		$this->setVal( 'fbAvatarConnectButton', '<fb:login-button perms="user_about_me" onlogin="UserProfilePage.fbConnectAvatar();">' . wfMessage('user-identity-box-connect-to-fb' )->escaped() . '</fb:login-button>' );
	...
	private function renderAboutLightbox( $userId ) {
		...
		$this->setVal( 'user', $userData );
		$this->setVal( 'fbConnectButton', '<fb:login-button perms="user_about_me,user_birthday,user_location,user_work_history,user_website" onlogin="UserProfilePage.fbConnect();">' . wfMessage( 'user-identity-box-connect-to-fb' )->escaped() . '</fb:login-button>' );
	...
	public function onFacebookConnectAvatar() {
		...
		$result = array( 'success' => false, 'error' => wfMessage( 'userprofilepage-interview-save-internal-error')->escaped() );
		$this->setVal( 'result', $result );
	...
	public function onFacebookConnect() {
		...
		if ( !$user->isAnon() ) {
			...
			if ( $fbUserId > 0 ) {
				...
				$result = array(
					'success' => false,
					'error' => wfMessage( 'user-identity-box-invalid-fb-id-error' )->escaped(),
				);
			}
		} else {
			$result = array(
				'success' => false,
				'error' => wfMessage( 'userprofilepage-interview-save-internal-error' )->escaped(),
			);
		}
