...
class SystemGiftManagerLogo extends SpecialPage {
  ...
  function SystemGiftManagerLogo(){
    UnlistedSpecialPage::UnlistedSpecialPage("SystemGiftManagerLogo");
  }
...
function createThumbnail($imageSrc, $ext,$imgDest,$thumbWidth){
	...
	function showSuccess($status) {
		...
		$output = "<h2>Upload Success</h2>";
		$output .= "<h5>Below are your images that will be used on the site</h5>";
		if($status==1)$ext = "gif";
		if($status==2)$ext = "jpg";
		if($status==3)$ext = "png";
		
		$output .= "<table cellspacing=0 cellpadding=5>";
		$output .= "<tr><td valign=top style='color:#666666;font-weight:800'>Large</td><td><img src={$wgUploadPath}/awards/sg_" . $this->gift_id . "_l." . $ext . "?ts=" .  rand()   . "></td></tr>";
		$output .= "<tr><td valign=top style='color:#666666;font-weight:800'>Medium-Large</td><td><img src={$wgUploadPath}/awards/sg_" . $this->gift_id . "_ml." . $ext . "?ts=" .  rand()   . "></td></tr>";
		$output .= "<tr><td valign=top style='color:#666666;font-weight:800'>Medium</td><td><img src={$wgUploadPath}/awards/sg_" . $this->gift_id . "_m." . $ext . "?ts=" .  rand()   . "></td></tr>";
		$output .= "<tr><td valign=top style='color:#666666;font-weight:800'>Small</td><td><img src={$wgUploadPath}/awards/sg_" . $this->gift_id . "_s." . $ext . "?ts" .  rand()   . "=></td></tr>";
		$output .= "<tr><td><input type=button onclick=javascript:history.go(-1) value='Go Back'></td></tr>";
		
		$output .= "<tr><td><a href=\"index.php?title=Special:SystemGiftManager\">Back to Gift List</a> |";
		$output .= " <a href=\"index.php?title=Special:SystemGiftManager&amp;id={$this->gift_id}\">Back to Edit This Gift</a></td></tr>";
		$output .= "</table>";
	...
	function uploadError( $error ) {
		...
		$wgOut->addHTML("<br><input type=button onclick=javascript:history.go(-1) value='Go Back'>");
	}
	...
	" ;
		...
		if($gift_image != ""){
			$output = "<table><tr><td style='color:#666666;font-weight:800'>Current Image</td></tr>";
			$output .= "<tr><td><img src=\"{$wgUploadPath}/awards/" . $gift_image . "\" border=\"0\" alt=\"gift\" /></td></tr></table><br>";
		}
		$wgOut->addHTML($output);
	...
	<td style='color:#666666;font-weight:800'>Your image must be a jpeg, png or gif (no animated gifs), and must be less than 100kb in size.<p>Choose File:<br>
