  ...
  return 'Zip File Function error: unknown';
	...
	switch ($provider) {
		case VideoPage::V_SCREENPLAY:
			$numCreated = self::importFromScreenplay($file);
			break;
...
public static function importFromScreenplay($file) {
	$articlesCreated = 0;
	...
	for ($i=0; $i<$numTitles; $i++) {
		...
		for ($j=0; $j<$numClips; $j++) {
			...
			$msg = '';
			$articlesCreated += self::createVideoPageForPartnerVideo(VideoPage::V_SCREENPLAY, $clipData, $msg);
			if ($msg) {
				print "ERROR: $msg\n";
			}
		}
...
public static function importFromMovieClips($id) {
	...
	foreach ($feed->get_items() as $key=>$item) {
		...
		$articlesCreated += self::createVideoPageForPartnerVideo(VideoPage::V_MOVIECLIPS, $clipData, $msg);
		if ($msg) {
			print "ERROR: $msg\n";
		}
	}
...
public static function createVideoPageForPartnerVideo($provider, array $data, &$msg) {
	global $debug;
