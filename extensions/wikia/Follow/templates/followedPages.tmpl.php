...
<?php foreach ($data as $key => $value): ?> 
	<?php if(empty($more)): ?>	
		<h2 class="firstHeading"><?php echo wfMsg($key , array("$1" => $value['count']) ) ?>
			<?php if($value['show_more']): ?>
				<a style="display:none;" class="ajax-show-more" href="<?php echo $wgServer.$wgScript."?action=ajax&rs=FollowHelper::showAll&head=".$key."&user_id=".$user_id ?>"><?php echo wfMsg('wikiafollowedpages-special-showall'); ?></a> 
			<?php endif;?>
		</h2> 
