...
//		}
	...
	private function makeSetFeedURL( $videoParams, $startDate, $endDate, $page ) {
		...
		if ( $videoParams['apiType'] == 'VideoAssets' ) {
			...
			$filter .= "(MediaId eq ".implode( ' or MediaId eq ', self::$MEDIA_IDS_MOVIE ).") ";
		} else {
			...
			if ( empty( $videoParams['isPromotesPublishedId'] ) ) {
				...
				if ( empty( $videoParams['isPublishedId'] ) ) {
					$filter .= "(substringof('$videoSet', Title) eq true) ";
				} else {
					$filter .= "(Publishedid eq $videoSet) ";
				}
			} else {
				$filter .= "(PromotesPublishedId eq $videoSet) ";
			}
