...
if( count( $ownerBadges ) == 0 ) {
			...
			foreach( $ownerBadges as $ownerBadge ) {
				...
				<li id="badge-<?= $i ?>">
					...
					<div id="hover-<?= $i ?>" class="profile-hover">
						<img width="90" height="90" src="<?= $ownerBadge[ 'badge' ]->getPictureUrl( 90 ) ?>"
							 alt="<?= htmlspecialchars( $ownerBadge[ 'badge' ]->getName() ) ?>"/>
					...
					<img id="badge-icon-<?= $i ?>" width="90" height="90"
						 src="<?= $ownerBadge[ 'badge' ]->getPictureUrl( 90 ) ?>"
						 alt="<?= htmlspecialchars( $ownerBadge[ 'badge' ]->getName() ) ?>"/>
				</li>
				<?php
			}
	...
	if( count( $ownerBadges ) > 6 ) {
		...
		<div id="sub-badges">
			<p>
				<button id="view-all"><?= wfMsg( 'achievements-viewall' ) ?></button>
				<button id="view-less" style="display:none;"><?= wfMsg( 'achievements-viewless' ) ?></button>
			</p>
		</div>
		<?php
	}
...
<ul id="challenges">
	...
	foreach( $challengesBadges as $badge ) {
		...
		<li class="clearfix">
			...
			<img width="40" height="40" src="<?= $badge[ 'badge' ]->getPictureUrl( 40 ) ?>"
				 alt="<?= $badge[ 'badge' ]->getName() ?>"/>
...
<div id="sub-badges-2">
	...
	?>
	<p><a href="<?= $leaderboard_url ?>" class="wikia-button"
		  id="achievements-leaderboard"><?= wfMsg( 'leaderboard-button' ) ?></a></p>
	<?php
	if( isset( $customize_url ) ) {
		?>
		<p><a href="<?= $customize_url ?>"
			  id="achievements-customize"><?= wfMsg( 'achievements-profile-customize' ) ?></a></p>
		<?php
	}
	?>
</div>
