...
?>
<form class="clearfix" action="<?= $wgScriptPath; ?>/index.php?action=ajax&amp;rs=AchAjax&amp;method=editPlatinumBadge"
	  method="POST" onsubmit="return SpecialCustomizePlatinum.editSubmit(this);" enctype="multipart/form-data">
	<fieldset class="customize-platinum-badge readonly">
		...
		<legend>
			...
			<!-- toggle group -->
			<input name="status" id="enabled-status" type="checkbox"
				   <?= $enabled ? 'checked="checked" ' : '' ?>value="1" disabled="disabled"/><label
				for="enabled-status"><?= wfMsg( 'achievements-community-platinum-enabled' ); ?></label>
		</legend>

		<div class="column description-fields"<?= ( $is_sponsored ) ? ' style="display:none;"' : null; ?>>
			<label><?= wfMsg( 'achievements-community-platinum-awarded-for' ); ?></label><span class="input-suggestion">&nbsp;(<?= wfMsg( 'achievements-community-platinum-awarded-for-example' ); ?>
				)</span>
			...
			<!-- toggle group -->
			<label><?= wfMsg( 'achievements-community-platinum-how-to-earn' ); ?></label><span class="input-suggestion">&nbsp;(<?= wfMsg( 'achievements-community-platinum-how-to-earn-example' ); ?>
				)</span>
		...
		<div class="sponsored-fields">
			<div class="clearfix sponsored-fields-1">
				<input type="checkbox" name="is_sponsored" value="1"
					   disabled="disabled" <?= ( $is_sponsored ) ? ' checked' : null; ?>/>
		...
		<div class="commands accent">
			<input type="button" value="<?= wfMsg( 'achievements-community-platinum-edit' ); ?>"
				   onclick="SpecialCustomizePlatinum.edit(this)" class="button-edit secondary"/>
			<input type="submit" value="<?= wfMsg( 'achievements-community-platinum-save' ); ?>" class="button-save"/>
			<input type="button" value="<?= wfMsg( 'achievements-community-platinum-cancel' ); ?>"
				   onclick="SpecialCustomizePlatinum.cancel(this)" class="button-cancel secondary"/>
		</div>
	</fieldset>
</form>
