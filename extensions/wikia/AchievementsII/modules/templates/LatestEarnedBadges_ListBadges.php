...
foreach( $badges as $item ) {
	...
	<li>
		<div
			class="profile-hover<?= ( $is_sponsored ) ? ' sponsored-hover' : null; ?>"<?= ( $is_sponsored && !empty( $hover_tracking_url ) ) ? " data-hovertrackurl=\"{$hover_tracking_url}\"" : null; ?>>
			<? if( $is_sponsored ) : ?>
				<img src="<?= $hover_url; ?>"/>
		...
		<a class="sponsored-link badge-icon badge-small"
			...
			<? endif; ?>
			<img
				class="<?= ( !$is_sponsored ) ? 'badge-icon ' : null; ?>badge-small badge-icon-<?= $count++ ?> badge-icon-earnable"
				width="40" height="40" src="<?= $badge_url; ?>" alt="<?= $badge_name; ?>">
			<? if( $is_sponsored ) : ?>
		</a>
	<? endif; ?>
