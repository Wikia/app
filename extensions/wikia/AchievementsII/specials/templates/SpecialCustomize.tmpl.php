<div class="article-sidebar">
	...
	<form method="POST" class="customize-edit-plus-category" onsubmit="Achievements.AchPrepareData(true);">
		...
		<p class="input">
			<label><?= wfMessage( 'achievements-customize-new-category-track' )->escaped(); ?></label><input
				name="edit_plus_category_name" type="text"/>
...
<div style="padding-right:320px;">
		...
		<div class="customize-section" id="section<?= $badgeTypeId; ?>">
			...
			<span class="enabled-flag">
				...
				<form method="POST" class="save-all">
					...
					<input type="hidden" name="editToken" value="<?= Sanitizer::encodeAttribute( $editToken ); ?>"/>
					<input type="submit" value="<?= wfMessage( 'achievements-save' )->escaped() ?>"
						   onclick="Achievements.AchPrepareData(false, '<?= $badgeTypeId; ?>');"/>
				</form>
			</div>
				...
				<? foreach( $trackData[ 'laps' ] as $lap => $badgeData ): ?>
					...
					<li class="<?= ( $lap == ( $inTrackBadgesCount - 1 ) ) ? ' last' : null ?>">
						...
						<div class="content-form">
							<p class="input">
								<input class="c-message" type="text" name="msg_<?= $badgeTypeId; ?>_<?= $lap; ?>"
									   value="<?= htmlspecialchars( $badge->getName() ); ?>">
							</p>
							<p><?= $config->getLevelName( $badge->getLevel() ); ?>
								(<?= wfMessage( 'achievements-points', $config->getLevelScore( $badge->getLevel() ) )->escaped(); ?>
								)</p>
						...
						<div class="image-form">
							...
							<span class="custom-text"><?= wfMessage( 'achievements-customize' )->escaped(); ?>
								<br/>
								<a href="#"
								   onclick="Achievements.revert(this, <?= $badgeTypeId; ?>, <?= $lap; ?>); return false;">
									<?= wfMessage( 'achievements-revert' )->escaped(); ?>
								</a>
							</span>
							</p>
							<form method="POST" enctype="multipart/form-data" class="customize-upload"
								  action="<?= $wgScriptPath ?>/index.php?action=ajax&amp;rs=AchAjax&amp;method=uploadBadgeImage&amp;type_id=<?= $badgeTypeId ?>&amp;lap=<?= $lap ?>&amp;level=<?= $badge->getLevel(); ?>"
								  onsubmit="return Achievements.submitPicture(this);">
								<p class="input">
		...
		<div class="customize-section" id="section<?= $section; ?>">
			...
			<div class="save-title-button">
				<form method="POST">
					...
					<input type="hidden" name="editToken" value="<?= Sanitizer::encodeAttribute( $editToken ); ?>"/>
					<input type="submit" value="<?= wfMessage( 'achievements-save' )->escaped() ?>"
						   onclick="Achievements.AchPrepareData(false, '<?= $section; ?>');"/>
				</form>
			</div>
			...
			<ul class="custom-badges">
				...
				<? foreach( $badges as $index => $badge ): ?>
					<li class="<?= ( $index == ( $badgesCount - 1 ) ) ? ' last' : '' ?>">
						<div class="content-form">
							<p class="input">
								<input class="c-message" type="text" name="msg_<?= $badge->getTypeId(); ?>"
									   value="<?= Sanitizer::encodeAttribute( $badge->getName() ); ?>">
							</p>
							<p><?= $config->getLevelName( $badge->getLevel() ); ?>
								(<?= wfMessage( 'achievements-points', $config->getLevelScore( $badge->getLevel() ) )->escaped(); ?>
								)</p>
						...
						<div class="image-form">
							<p><img width="90" height="90" src="<?= $badge->getPictureUrl( 90 ); ?>"/><br/><span
									class="custom-text"><?= wfMessage( 'achievements-customize' )->escaped(); ?><br/><a
										href="#"
										onclick="Achievements.revert(this, <?= $badge->getTypeId(); ?>); return false;"><?= wfMessage( 'achievements-revert' )->escaped(); ?></a></span>
							</p>
							<form method="POST" enctype="multipart/form-data" class="customize-upload"
								  action="<?= $wgScriptPath ?>/index.php?action=ajax&amp;rs=AchAjax&amp;method=uploadBadgeImage&amp;type_id=<?= $badge->getTypeId(); ?>&amp;level=<?= $badge->getLevel(); ?>"
								  onsubmit="return Achievements.submitPicture(this);">
								<p class="input">
									<input name="wpUploadFile" type="file"/>
	...
	<form method="POST" class="save-all">
		...
		<input type="hidden" name="editToken" value="<?= Sanitizer::encodeAttribute( $editToken ); ?>"/>
		<input type="submit" value="<?= wfMessage( 'achievements-save' )->escaped() ?>"
			   onclick="Achievements.AchPrepareData(false, '');"/>
	</form>
</div>
<? if( $scrollTo ): ?>
	<script type="<?= $wgJsMimeType; ?>">
		setTimeout(function () {
			$(window).scrollTo('#section<?= $scrollTo ?>', 2000);
		}, '3000');
	</script>
<? endif; ?>
