...
function setLightboxText(message){
	...
	objImage.style.display = 'none';
	Element.setOpacity(objLightboxText, 0.01);
	objLightbox.style.display = 'block';
	...
	// center lightbox and make sure that the top and left values are not negative
	// and the image placed outside the viewport
	var lightboxTop = arrayPageScroll[1] + ((arrayPageSize[3] - 35 - Element.getHeight('lightboxText')) / 2);
	var lightboxLeft = ((arrayPageSize[0] - 20 - Element.getWidth('lightboxText')) / 2);	
	
	objLightbox.style.top = (lightboxTop < 0) ? "0px" : lightboxTop + "px";
	objLightbox.style.left = (lightboxLeft < 0) ? "0px" : lightboxLeft + "px";
	Element.setOpacity(objLightboxText, 1.00);
}
