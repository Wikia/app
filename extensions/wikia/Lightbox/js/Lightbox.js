...
(function (window, $) {
	...
	var Lightbox = {
		...
		makeLightbox: function (params) {
			...
			$(window)
				.trigger('lightboxOpened');

			// if we don't have latest photos in the DOM, request them from back end
			Lightbox.includeLatestPhotos = !($('#LatestPhotosModule')
		...
		clearErrorMessageStyling: function () {
			this.openModal.media.removeClass('error-lightbox')
				.attr('style', 'line-height: normal;');
		},
		image: {
			...
			updateLightbox: function (data) {
				Lightbox.image.getDimensions(data.imageUrl, function (dimensions) {
					...
					Lightbox.openModal.media.find('img')
						.first()
						.load(function () {
							$(window)
								.trigger('resize'); // firefox image loading hack (BugId:32477)
						});
		...
		ads: {
			...
			updateLightbox: function () {
				...
				Lightbox.openModal.carousel.find('.active')
					.removeClass('active');
		...
		error: {
			updateLightbox: function () {
				...
				Lightbox.openModal.header
					.html('')
					.prepend($(Lightbox.openModal.closeButton)
						.clone(true, true)); // clone close button into header
		...
		updateUrlState: function (clear) {
			...
			if (clear) {
				...
				qs.setVal('file', this.current.key, true)
					.replaceState();
			}
		},
		...
		setUpCarousel: function () {
			...
			var trackProgressCallback = function (idx1, idx2, total) {
				...
				var firstThumb = Lightbox.openModal.carousel.find('li')
					.eq(idx1);
			...
			var beforeMove = function () {
				Lightbox.openModal.carousel.find('.Wikia-video-play-button .play')
					.hide();
			};
			...
			$.when.apply(this, deferredList)
				.done(function () {
					...
					var placeholder = $('#LightboxCarouselMore')
						.mustache({
							text: Lightbox.backfillCountMessage
						});
					Lightbox.openModal.carousel.find('li')
		...
		getMediaThumbs: {
			...
			articleMedia: function (backfill) {
				...
				if (cached.length) {
					...
					if (!thumbs.length) {
						thumbs = article.find('.image, .lightbox')
							.find('img')
							.add(article.find('.thumbimage'));
					...
					thumbs.each(function () {
						...
						if ($thisThumb.closest('.ogg_player')
							.length) {
							return;
						}

						videoName = $thisThumb.attr('data-video-name') || $thisThumb.parent()
							.attr('data-video-name');
						...
						if (videoName) {
							...
							type = 'image';
							title = $thisThumb.attr('data-image-name') || $thisThumb.parent()
