...
{var CKEDITOR={timestamp:'',version:'3.4.2',revision:'r31058 build 20101209',_:{},status:'unloaded',basePath:(function()
...
{var nonExitableElementNames={table:1,pre:1};var emptyParagraphRegexp=/(^|<body\b[^>]*>)\s*<(p|div|address|h\d|center)[^>]*>\s*(?:<br[^>]*>|&nbsp;|\u00A0|&#160;)?\s*(:?<\/\2>)?\s*(?=$|<\/body>)/gi;var notWhitespaceEval=CKEDITOR.dom.walker.whitespaces(true);function checkReadOnly(selection)
...
{if(this.mode=='wysiwyg')
{this.focus();var selection=this.getSelection();if(checkReadOnly(selection))
return;this.fire('saveSnapshot');var ranges=selection.getRanges(),element=evt.data,elementName=element.getName(),isBlock=CKEDITOR.dtd.$block[elementName];var selIsLocked=selection.isLocked;if(selIsLocked)
...
{var config=editor.config['toolbar_'+editor.config.toolbar];var messages=RTE.instance.lang.bucket;var output=['<table id="cke_toolbar"><tr>'];if(window.skin=='oasis'){var colorClass='';}
else{var colorClass=' color1';}
for(var b=0;b<config.length;b++)
{var bucket=config[b];output.push('<td><span class="headline'+colorClass+'">'+messages[bucket.msg]+'</span>');output.push('<div class="bucket_buttons'+colorClass+'"><div class="'+colorClass+'">');for(var g=0;g<bucket.groups.length;g++)
{var items=bucket.groups[g];var itemsCount=items.length;if(items==false){continue;}
output.push('<span class="cke_buttons_group">');for(var i=0;i<itemsCount;i++){var itemName=items[i];var item=editor.ui.create(itemName);if(item)
...
if(hidden>0){cell.find(".tagline").html(hidden+' '+RTE.messages.more);cell.addClass('more');}else{cell.find(".tagline").html('<div> </div>');cell.removeClass('more');}});if(anyhidden){$('#cke_toolbar').addClass('more');}else{$('#cke_toolbar').removeClass('more');}
