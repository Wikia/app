define( 'mediaGallery.templates.mustache', [], function() { 'use strict'; return {
    "MediaGallery_gallery" : '{{! maybe recreate this stuff in JS or maybe not }}<h1>FOOOOOO</h1><div class="media-gallery-wrapper count-{{count}}" data-visible-count="{{visibleCount}}"><script>window.Wikia = window.Wikia || {};Wikia.mediaGalleryData = Wikia.mediaGalleryData || [];Wikia.mediaGalleryData.push({{{json}}});</script><button class="add-image">{{addImageButton}}</button><div class="media-gallery-inner"><noscript>{{#media}}<a href="{{linkHref}}"><img src="{{thumb}}" alt="{{{caption}}}"></a>{{/media}}</noscript></div></div>',
    "MediaGallery_media" : '<div class="media {{classes}}">{{{thumbHtml}}}{{#caption}}<div class="caption"><div class="inner">{{{caption}}}</div></div>{{/caption}}</div>',
