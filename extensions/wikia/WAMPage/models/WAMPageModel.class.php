...
class WAMPageModel extends WikiaModel {
	...
	public function getIndexWikis($params) {
		$emptyData = ( isset($params['shownoresults']) ) ? true : false;
		$params = $this->getIndexParams($params);
		
		if( !empty($this->app->wg->DevelEnvironment) ) {
			$WAMData = $this->getMockedDataForDev( $emptyData );
		} else {
			$WAMData = $this->app->sendRequest('WAMApi', 'getWAMIndex', $params)->getData();
		}
	 ...
	 * 
	 * @param boolean $emptyData if set to true the mocked data will be "empty"
	 *
	 * @return array
	 */
	protected function getMockedDataForDev($emptyData = false) {
		if( $emptyData ) {
			return ['wam_results_total' => 0, 'wam_index' => []];
		} else {
