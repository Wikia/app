...
describe('ext.wikia.PaidAssetDrop', function () {
	...
	it('today is not valid when wgPADConfig is not set', function () {
		expect(getModule().isTodayValid()).toEqual(false);
	});

	it('today is not valid when wgPADConfig start day is not set', function () {
		mocks.win.wgPADConfig = [];
		expect(getModule().isTodayValid()).toEqual(false);
	});

	it('today is not valid when wgPADConfig start day is not set', function () {
		mocks.win.wgPADConfig = ['2015-04-12'];
		expect(getModule().isTodayValid()).toEqual(false);
	});

	it('today is not valid when it is before start date', function () {
		mocks.win.wgPADConfig = ['2015-04-14', '2015-04-21'];
	...
	it('today is not valid when it is before start date (case with time)', function () {
		mocks.win.wgPADConfig = ['2015-04-14T12:00:00Z', '2015-04-14T20:00:00Z'];
	...
	it('today is invalid when both dates are invalid', function () {
		mocks.win.wgPADConfig = ['foo', 'bar'];
		expect(getModule().isTodayValid()).toEqual(false);
	});

	it('today is invalid when end date is invalid', function () {
		mocks.win.wgPADConfig = ['2015-04-20', 'bar'];
		expect(getModule().isTodayValid()).toEqual(false);
	});
});
