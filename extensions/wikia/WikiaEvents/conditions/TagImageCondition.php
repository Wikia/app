		...
		else
		{
			...
			if ( class_exists('WikiaEventsConditions') ) 
			{
				...
				if ($eventsCond)
				{
					...
					{
						wfDebug( "Execute TagImageCondition for user:" . $wgUser->getName(). " (" . $wgUser->getID() . ")");
						$eventsCond->executeCondition(1);
					}
					else
					{
						wfDebug( "Don't use TagImageCondition class for user:" . $wgUser->getName(). " (" . $wgUser->getID() . ")");
 ...
 * for PHP < 5.2.x
 */
function wfTagImageCondition($ev_id, &$out)
{
	return TagImageCondition::execute($ev_id, &$out);
}
