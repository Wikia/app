		...
		else
		{
			...
			if ( class_exists('WikiaEventsConditions') ) 
			{
				...
				if ($eventsCond)
				{
					...
					{
						wfDebug( "Execute SendToFriendCondition for user:" . $user->getName(). " (" . $user->getID() . ")");
						$eventsCond->executeCondition();
					}
					else
					{
						wfDebug( "Don't use SendToFriendCondition for user:" . $user->getName(). " (" . $user->getID() . ")");
 ...
 * for PHP < 5.2.x
 */
function wfSendToFriendCondition($ev_id, &$article, &$user, $text, $summary, $minor_flag, $opt1, $opt2, &$flags)
{
	return SendToFriendCondition::execute($ev_id, &$article, &$user, $text, $summary, $minor_flag, $opt1, $opt2, &$flags);
}
