...
* Changes 
***************************************************************************************************
* 2008-02-10 : Andrewy prepended wikia to method calls not to clash with other similar methods
* 2003-06-10 : GuinuX 
...
* class http_header
******************************************************************************************/
	class wikia_http_header {
		var $_headers;
		var $_debug;

		function wikia_http_header() {
			$this->_headers = Array();
			$this->_debug	= '';
...
******************************************************************************************/
	class wikia_http_response_header extends wikia_http_header {
		var $cookies_headers;
		
		function wikia_http_response_header() {
			$this->cookies_headers = array();
			wikia_http_header::wikia_http_header();
...
******************************************************************************************/
	class wikia_http_request_message extends wikia_http_header {
		var $body;
...
******************************************************************************************/
	class wikia_http_response_message extends wikia_http_response_header {
		var $body;
		var $cookies;
		
		function wikia_http_response_message() {
			$this->cookies = new wikia_http_cookie();
			$this->body = '';
			wikia_http_response_header::wikia_http_response_header();
		...
		function reset() {
			$this->body = '';
			wikia_http_response_header::reset();
		}
...
* class http_cookie
******************************************************************************************/
	class wikia_http_cookie {
		var $cookies;

		function wikia_http_cookie() {
			$this->cookies 	= array();
		...
		function get( $current_domain, $current_path ) {
			...
			foreach( $this->cookies as $cookie_name => $cookie_data ) {
				if ($cookie_data['expires'] > $now) {
					...
					if ( preg_match( "'.*$domain$'i", $current_domain )){
...
******************************************************************************************/
	class wikia_http {
		...
		function wikia_http( $http_version = HTTP_V10, $keep_alive = false, $auth = false ) {
			$this->http_version	= $http_version;
			$this->connected	= false;
			$this->user_agent	= 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.8.1.9) Gecko/20071025 Firefox/2.0.0.9';
			$this->host			= '';
			$this->port			= 80;
			$this->errstr		= '';
			...
			$this->_response 	= new wikia_http_response_message();
			$this->_request		= new wikia_http_request_message();
