...
if (skin == "monaco") {
	$("h2 span.mw-headline").each(function(i) {
		...
		$(this).parent().prepend("<span class=\"editsection\"><a href=\"'.$editUrl.'\" onclick=\"WET.byStr(\'articleAction/editSection\')\">'.$editMsg.'</a></span>").find(".editsection a").addClass("wikia_button").contents().wrap("<span></span>").end().parent().css("position", "relative").css("top", "-3px").css("margin-bottom", "-2px");
	});
}';
	}
	else {
...
if (skin == "monaco") {
	$("h2 span.editsection").each(function(i) {
		...
		//add elements and styling
		$(this).find("a").click(function(){WET.byStr(\'articleAction/editSection\')}).addClass("wikia_button").contents().wrap("<span></span>").end().parent().css("position", "relative").css("top", "-3px").css("margin-bottom", "-2px");
	});
}';
	}
