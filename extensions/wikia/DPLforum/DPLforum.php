...
class DPLForum
{
  ...
  function parse(&$input, &$parser)
  {
    ...
    if($sPrefix==='' && (($cats < 1 && $iNamespace < 0) ||
        ($total < $this->minCategories))) return $this->msg('forum_toofew',1);
    if(($total > $this->maxCategories) && (!$this->unlimitedCategories))
        return $this->msg('forum_toomany',1);
    ...
    else if(is_null($title))
    {
      while($row = $dbr->fetchObject( $res ))
      {
        ...
        $output .= $sStartItem;
        $first_user = (!empty($first_time)) ? $row->first_user : "";
        $first_time = (!empty($first_time)) ? $row->first_time : "";
        $output .= $this->buildOutput($title, $title, $row->rev_timestamp, $row->rev_user_text, $first_user, $first_time);
        $output .= $sEndItem . "\n";
      }
    }
    else
    {
      ...
      {
        $output .= $this->buildOutput(Title::makeTitle($row->page_namespace, $row->page_title), $title, $row->rev_timestamp, $row->rev_user_text);
      }
      else
      {
  ...
  function buildOutput($page, $title, $time, $user='', $author='', $made='')
  {
    ...
    if($tm) $output .= "</td>";
    else if($text) $output .= $this->msg('forum_edited') . " $text";
    return $output;
  }
}
