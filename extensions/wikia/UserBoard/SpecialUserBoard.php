	...
	class ViewUserBoard extends SpecialPage {
		...
		function execute(){
			...
			if($wgUser->getID() == 0 && $user_name==""){
				...
				$wgOut->redirect( $login->getLocalURL("returnto=Special:UserBoard") );
				return false;
			}
			...
			if($can_post){	
				if ( $wgUser->isLoggedIn() && !$wgUser->isBlocked() ) {
					$output .= '<div class="user-page-message-form">
						...
						<select id="message_type"><option value="0">' . wfMsg("userboard_public") . '</option>
						<option value="1">' . wfMsg("userboard_private") . '</option></select><p>
