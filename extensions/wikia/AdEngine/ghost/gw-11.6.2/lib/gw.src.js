...
function ghostwriter(element, options){
	...
	return ghostload(element,options);
}
...
function ghostload(element, options){
	...
	function loadNext(){
		...
		var nextScript= loadqueue.shift()
...
function find(){
	...
	while(a.length > 0){ 
		var 
		  ...
		  ,isjs= type.indexOf(JS) > 0 ||( lang && lang.indexOf(JS) > -1 )|| !type
...
this['ghostwriter']['capabilities']= (function(document){
	...
	function testSynchronousExec(){
		...
		$anchor.parentNode.insertBefore(asyncBlocker,$anchor)
		$anchor.parentNode.insertBefore(synchronous,$anchor)
