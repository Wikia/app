...
window['ghostwriter']= window['ghostwriter'] || (function(){
var 
	 D= document
	,W= window
	...
	,_DBG= W.location.hash.toLowerCase().indexOf("gwrdebug") >= 0 ? true : false
	,MESSAGES= (function($){ 
		for(var m in $)
			$[m]= ["ghostwriter.js  >> ", $[m],""].join("");
		return $; 
	})({ 
		 queue:"Queueing ghostwriter load"
		,start:"Begin ghostwriter load"
		,done:"Finished ghostwriter load" 
...
function ghostwriter(element, options){ 
	...
	if(R){ 
		...
		if(_DBG){(function(scriptId){ 
			flog([MESSAGES.queue,scriptId,""].join(" '"));
		})(getscriptidentifier(options['script']));
		}
		return false; 
	}else
...
function ghostload(element, options){ 
	...
	; 
	if(_DBG)(function(scriptId){ 
		flog([MESSAGES.start,scriptId,""].join(" '"));
	})(getscriptidentifier(curr));
	...
	function onload(complete){
		...
		if(_DBG && !complete)(function(scriptId){ 
			flog([MESSAGES.finish,scriptId,""].join(" '"));
		})(getscriptidentifier(curr));
		...
		if(loadqueue.length > 0){ 
			...
			if(_DBG)(function(scriptId){ 
				flog([MESSAGES.attach,scriptId,""].join(" '"));
			})(getscriptidentifier(curr));
			insertfn(curr); 
		}else if (FF && !complete){ 
...
function getscriptidentifier(script){ 
	if(!script) 
		return ""; 
	var 
		 scriptText= script.src ? script.src : script.text
		,scriptId= script.id || ""
		,charCount= 20
	...
	scriptText= scriptText.replace(whiteSpaceRegex, ""); 
...
return flog; 
function flog(msg){ 
	var diff= ((new Date() - startTime) / 1000).toFixed(1);
...
,_RS= new Function() 
; 
flushloadhandlers['set']= setoverrides; 
flushloadhandlers.fns= _owQ;
 ...
 */
	...
	,create: (function(){ 
	var get= IE && (!document.documentMode || document.documentMode < 9) ? 
		function(a,b){ 
			var c,d=["<" + a];
			for(var e in b) 	
				d[d.length]= e + '="' + b[e] + '"';
...
function parse(html){
	...
	while (html){
		...
		if(!lastTag || !special[lastTag]){
			if(html.indexOf(comments.startix)===0){
				if((index= comments.endre.exec(html))){ 
					if(index[0].length > 3) 
	...
	function findTagBody(all, text){
		...
		;
		var i= text.lastIndexOf(comments.endix); 
