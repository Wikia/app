...
function newAuction(_ref) {
  ...
  return {
    ...
    getBidsReceived: function getBidsReceived() {//console.log('aaa');
...
function newAuctionManager() {
  ...
  auctionManager.getBidsReceived = function () {
    ...
    var cos = _auctions.map(function (auction) {
      var stat = auction.getAuctionStatus();//debugger;
      if (stat === _auction.AUCTION_COMPLETED) {
        var auc = auction.getBidsReceived();//debugger;
        return auc;
      }
    });

    cos = cos.reduce(_utils.flatten, []).filter(function (bid) {
      return bid;
    });
    //debugger;
    return cos;
...
function isValidVideoBid(bid, bidRequests) {
  ...
  if (context === OUTSTREAM) {
    ...
    return !!(bid.renderer || bidRequest.renderer || bid.vastUrl === 1);
  }
...
function newBid(serverBid, rtbBid, bidderRequest) {
  ...
  if (rtbBid.rtb.video) {
    _extends(bid, {
      ...
      // WIKIA_CHANGE: When AppNexus renderer is disabled and vastUrl is not set - fallback to 1 instead of undefined
      vastUrl: ENABLE_VIDEO_RENDERER ? rtbBid.rtb.video.asset_url : 1,
