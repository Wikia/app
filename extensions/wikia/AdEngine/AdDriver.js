	...
	try {
		...
		if (window.wgAdDriverUseExpiryStorage) {
			...
			if (storageNum != cookieNum) {
				WikiaTracker.track([LiftiumOptions.pubid, "error", "numcalloutofsync", storageName, slotname, Liftium.dbnameForTracking(window.wgDBname), storageNum, cookieNum], 'liftium.errors');				
	...
	try {
		...
		if (window.wgAdDriverUseExpiryStorage) {
			if (storageValue != cookieValue) {
				WikiaTracker.track([LiftiumOptions.pubid, "error", "lastdartcallnoadoutofsync", slotname, Liftium.dbnameForTracking(window.wgDBname)], 'liftium.errors');
			}
		}		
	}
	catch (e) {
