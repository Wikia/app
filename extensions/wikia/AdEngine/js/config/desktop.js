...
define('ext.wikia.adEngine.config.desktop', [
	...
	function getProviderList(slotName) {
		var providerList = [],
			isPrecious = lookup && lookup.isPrecious(slotName);
		...
		if (!isPrecious && context.providers.turtle && adProviderTurtle.canHandleSlot(slotName)) {
			providerList.push(adProviderTurtle);
		} else if (!isPrecious && context.providers.evolve2 && adProviderEvolve2.canHandleSlot(slotName)) {
			providerList.push(adProviderEvolve2);
		} else if (!isPrecious && evolveCountry && adProviderEvolve.canHandleSlot(slotName)) {
