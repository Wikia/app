...
describe('ext.wikia.adEngine.monetizationsServiceHelper', function () {
	'use strict';

	var mocks, monetizationsServiceHelper;
	...
	monetizationsServiceHelper = modules['ext.wikia.adEngine.monetizationsServiceHelper'](
		jQuery,
		mocks.geo,
		mocks.scriptwriter,
		mocks.tracker
	...
	Object.keys(testCasesGetMaxAds).forEach(function (key) {
		it('Get maximum number of ads. Test: ' + key, function () {
			...
			expect(monetizationsServiceHelper.getMaxAds()).toEqual(testCasesGetMaxAds[key].expect);
		});
	});
	...
	Object.keys(testCasesGetCountryCode).forEach(function (key) {
		it('Get country code for the service. Test: ' + key, function () {
			...
			expect(monetizationsServiceHelper.getCountryCode()).toEqual(testCasesGetCountryCode[key].expect);
		});
	});
});
