var AccordionMenu = function()
{
	...
	function expand(dl,dt,dd,wa/*without animation*/)
	{
		var sectionHeaders = dl.getElementsByTagName('dt');
		var titleHeight = sectionHeaders[0].offsetHeight;
		var sectionHeaderHeight = sectionHeaders[1].offsetHeight;
		var accordionHeight = parseInt(YUD.getStyle(dl, 'height'));
		
		var sectionHeight = accordionHeight - titleHeight - (sectionHeaderHeight * (sectionHeaders.length - 1));
		
		dl.hasAnimation +=1;
		YUD.addClass(dd,'a-m-d-before-expand');     
		var oAttr = {height:{from:(wa ? sectionHeight : 0),to:sectionHeight }};
