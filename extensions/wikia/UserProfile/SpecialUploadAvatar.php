...
class UploadAvatar extends SpecialPage {
  ...
  function execute(){
	  ...
	  $out .= $wgOut->setPagetitle( "Upload Avatar" );
  ...
  function initAvatar(&$request){
  	...
  	function getAvatar($size){
		...
		$files = glob(dirname( __FILE__ ) . "/../images/avatars/" . $wgDBname . "_" . $wgUser->mId .  "_" . $size . "*");
		if($files[0]){
			return "<img src=images/avatars/" .  basename($files[0]) . ">" ;
		}
	}
...
function createThumbnail($imageSrc, $ext,$imgDest,$thumbWidth){
	...
	function showSuccess($status) {
		global $wgUser, $wgOut, $wgContLang, $wgDBname;
		$ext = "jpg";
		...
		$output .= "<tr><td valign=top style='color:#666666;font-weight:800'>Large</td><td><img src=images/avatars/" . $wgDBname . "_" . $wgUser->mId . "_l." . $ext . "?ts=" .  rand()   . "></td></tr>";
		$output .= "<tr><td valign=top style='color:#666666;font-weight:800'>Medium</td><td><img src=images/avatars/" . $wgDBname . "_" . $wgUser->mId . "_m." . $ext . "?ts=" .  rand()   . "></td></tr>";
		$output .= "<tr><td valign=top style='color:#666666;font-weight:800'>Small</td><td><img src=images/avatars/" . $wgDBname . "_" . $wgUser->mId . "_s." . $ext . "?ts" .  rand()   . "=></td></tr>";
	...
	" ;
		...
		if($this->getAvatar("l") != ""){
			...
			$output .= "<tr><td>" . $this->getAvatar("l") . "</td></tr></table><br>";
		}
		$wgOut->addHTML($output);
