...
function WidgetAnswers_handler(e) {
	if(e.type == 'focus') {
		...
		if(keycode == 13 && e.target.value != '' ) {
			window.open('http://' + wgContentLanguage + '.answers.wikia.com/index.php?title=Special:CreateQuestionPage&questiontitle=' + encodeURIComponent(e.target.value) + '&categories=' + wgSitename, 'wikianswers');
			e.target.value = widget_answers_placeholder;
			e.target.style.color = '#999';
		}
	}
