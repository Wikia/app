...
$wgExtensionCredits['other'][] = array(
	'name' => 'Akismet',
	'description' => 'akismet anti-spam web service integration module',
	'author' => 'Tomasz Klim'
);
...
class Akismet
{
	...
	private function http_post($request, $host, $path) {
		...
		$response = $this->send_request( $host, $this->apiPort, $http_request );

		return explode( "\r\n\r\n", $response, 2 );
	}

	private function send_request( $host, $port, $request, $responseLength = 1160 ) {
		$errorNumber = 0;
		$errorString = '';
		$response = '';
		...
		if ( $errorNumber != 0 ) {
			throw new Exception( 'Error connecting to host: ' . $host . ' Error number: ' . $errorNumber . ' Error message: ' . $errorString );
		}
