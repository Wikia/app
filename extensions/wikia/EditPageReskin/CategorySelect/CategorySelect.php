...
wgAfterContentAndJS.push(function() {
	...
	$('#csAddCategorySwitch').children('a').click(function(ev) {
		...
		$.getResources([
			...
			$.loadYUI,
			wgExtensionsPath + '/wikia/EditPageReskin/CategorySelect/CategorySelect.js?' + wgStyleVersion,
			(window.skin == 'oasis' ? $.getSassCommonURL('/extensions/wikia/EditPageReskin/CategorySelect/oasis.scss') : wgExtensionsPath + '/wikia/EditPageReskin/CategorySelect/CategorySelect.css?' + wgStyleVersion)
		],
		function() {
			csTrack('addCategory');
			showCSpanel();
...
function CategorySelectGenerateHTMLforEdit($formId = '') {
	...
	if (Wikia::isOasis()) {
		$cssFile = AssetsManager::getInstance()->getSassCommonURL('/extensions/wikia/EditPageReskin/CategorySelect/oasis.scss');
	}
	else {
		$cssFile = "{$wgExtensionsPath}/wikia/EditPageReskin/CategorySelect/CategorySelect.css?{$wgStyleVersion}";
	}
