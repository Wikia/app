...
var listUsersLanguage = {
    ...
    "sInfo": "<?= wfMessage('listglobalusersrecordspager', '_START_', '_END_', '_TOTAL_')->parse() ?>",
    "sInfoEmpty": "<?= wfMessage( 'listglobalusersrecordspager','0', '0', '0' )->parse() ?>",
    "sInfoFiltered": "",
...
$( function () {
	$( '#lu-select-all' ).click( function () {
		...
		if ( $this.hasClass( 'lu-selected' ) ) {
			...
			$this.val( '<?= wfMessage( 'listglobalusers-select-all' )->escaped() ?>' );
		} else {
			$checkbox.prop( 'checked', true );
			$this.addClass( 'lu-selected' );
			$this.val( '<?= wfMessage( 'listglobalusers-deselect-all' )->escaped() ?>' );
		}
	} );
} );
...
<legend><?= wfMessage( 'listglobalusers-groups' )->escaped() ?></legend>
	<table><tr>
<?
	$i = 0;
...
?>
		<td valign="middle" style="padding:0px 2px 0px 1px;">
			<label for="checkBoxFor<?=$groupName?>">
				...
				<span style="padding-bottom:5px;"><?= $link ?> <small>(<?= wfMessage( 'listglobaluserscount', ( isset( $group['count'] ) ) ? intval($group['count']) : 0 )->parse() ?>)</small></span>
			</label>
		</td>
<?
		$i++;
...
?>
		...
		<tr><td colspan="4" style="text-align:right;"><input type="button" class="wikia-button lu-selected" id="lu-select-all" value="<?= wfMessage( 'listglobalusers-deselect-all' )->escaped() ?>" /></td>
	</table>
</fieldset>
<? } ?>
