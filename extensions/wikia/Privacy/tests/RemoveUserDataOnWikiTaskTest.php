...
class RemoveUserDataOnWikiTaskTest extends WikiaDatabaseTest {
	...
	public function testShouldRemoveUserData() {
		( new RemoveUserDataOnWikiTask() )->removeUserDataOnCurrentWiki( self::TEST_AUDIT_ID,
			self::TEST_USER_ID );
	...
	public function testShouldRemoveAbuseFilterData() {
		...
		$log =
			$dbr->select( 'abuse_filter_log', [ 'afl_user' ],
				[ 'afl_user' => self::TEST_USER_ID ] );
	...
	public function testShouldRemoveUserPageHistoryFromRecentChanges() {
		...
		foreach ( $changes as $change ) {
			$isUserPage =
				in_array( $change->rc_namespace, RemoveUserDataOnWikiTask::USER_NAMESPACES );
	...
	public function testShouldRemoveWatchlist() {
		...
		$this->assertEquals( 0,
			$db->selectField( 'watchlist', 'count(*)', [ 'wl_user' => self::TEST_USER_ID ],
				__METHOD__ ), 'Watchlist is not empty' );
	}
	...
	public function testShouldHaveAuditLog() {
		...
		$this->assertEquals( 1, $dbr->selectField( 'rtbf_log_details', 'count(*)',
			[ 'log_id' => self::TEST_AUDIT_ID, 'was_successful' => 1 ], __METHOD__ ),
			'No audit log found' );
	}
