...
function wfProfileJSON($user_name, $r_user_name="") {
	...
	if( $user_id  == 0 && !User::isIP($user_name) ){
		...
		$text = "var json_profile=" . jsonify($profile_JSON_array) . ";\n\nprofile_from_JSON(json_profile);";
...
function wfProfileBlockJSON( $user_id ) {
	...
	$response = new AjaxResponse( "window.location='avoid.html'" );
	$response->setContentType( "application/javascript; charset=utf-8" ); 
...
function wfGetProfileBlocksJSON( $callback="showUsers" ){  
	...
	$text =  "var json_blocked=" . jsonify($profile_JSON_array) . ";\n\n{$callback}(json_blocked);";
	$response = new AjaxResponse( $text );
	$response->setContentType( "application/javascript; charset=utf-8" ); 
...
function wfDeleteProfileBlocksJSON( $user_id ){  
	...
	$response = new AjaxResponse( "window.location.reload();" );
	$response->setContentType( "application/javascript; charset=utf-8" ); 
	return $response;
}
...
function wfgetNotificationsJSON($callback="display_notifications") {
	...
	$text = "var notifications=" . jsonify($notifications_array) . ";\n\n{$callback}(notifications);";
	$response = new AjaxResponse( $text );
	$response->setContentType( "application/javascript; charset=utf-8" ); 
	$response->setCacheDuration( 60 * 10 );
...
function wfClearNewMessageCountJSON() {
	...
	$response = new AjaxResponse( "//cleared new message count" );
	$response->setContentType( "application/javascript; charset=utf-8" ); 
	return $response;
}
...
function wfCheckNewNotificationsJSON( ) {
	...
	$response = new AjaxResponse( "//no new notifications" );
	$response->setContentType( "application/javascript; charset=utf-8" ); 
	$response->setCacheDuration( 60 * 10 );
...
function wfUpdateProfileBGJSON($user_name, $bg, $scroll=0) {
	...
	$response = new AjaxResponse( "//updated background" );
	$response->setContentType( "application/javascript; charset=utf-8" ); 
	return $response;
}
