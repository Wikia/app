<?php
	...
	function process_messages_forJSON($messages, $r_user_name="") {
		foreach($messages as $message=>$message_stuff) {
				...
				//$message_stuff["user_from_link"] =  Title::makeTitle( NS_USER  , $message_stuff["user_name_from"]  )->escapeFullUrl();
				...
				if($r_user_name!=$message_stuff["user_name_from"]){
					//$message_stuff["board_to_board"] = UserBoard::getUserBoardToBoardURL($message_stuff["user_name"],$message_stuff["user_name_from"]);
					if ($message_stuff["user_name"] == $message_stuff["user_name_from"]) {
						...
						//if ($message_stuff["type"] == 1) {
							//$message_stuff["board_to_board"] += "&type=".$message_stuff["type"];
						//}
					}
					//$message_stuff["board_link"] = UserBoard::getUserBoardURL($message_stuff["user_name_from"]);
...
function wfSendBoardMessageJSON(){ 
	...
	if($wgRequest->wasPosted()) {
		...
		//$messages = $b->displayMessages($user_id_to,0,$count);
		...
		//return "var new_messages = " . jsonify($messages_full) . ";\n\nhandle_posted_message(new_messages);";
	...
	function getBoardMessagesJSON($user_name, $r_user_name="") {
		...
		if ($wgUserProfileDisplay['board'] != false) {
			...
			return "var json_board=" . jsonify($board_JSON_array) . ";\n\nboard_from_JSON(json_board);";
		}
		
		return "//board not enabled";
	...
	function doGetBoardMessagesJSON ($user_name, $r_user_name="", $check_count=10, $from_notifications=false) {
		...
		if ($wgUserProfileDisplay['board'] != false) {
			...
			//if (!$user->getID()) {
				//return jsonify("Not a valid Username");
			//}
	...
	$wgAjaxExportList [] = 'wfDeleteBoardMessageJSON';
		 function wfDeleteBoardMessageJSON($ub_id){ 
		  ...
		  //return "getMessages()";
		  return "\$('profile-message-{$ub_id}').innerHTML=\"\";\n\n\$('profile-message-{$ub_id}').toggle();\n\nupdate_total();";
	...
	function getBoardtoBoardMessagesJSON($user_name, $user_name_2, $page) {
	 global $wgUser, $wgOut, $wgRequest, $IP, $wgMemc, $wgStyleVersion, $wgUserBoardScripts;
			...
			if($wgUser->getID() == 0 && $user_name==""){
				// handle not logged-in and no user
			}
