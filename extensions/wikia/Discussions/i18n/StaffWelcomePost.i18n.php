<?php
$messages = array();

$messages['en'] = array(
	'discussions-staff-welcome-title' => 'Welcome to Discussions!',
	'discussions-staff-welcome-post' => "
This is the place to talk about your favorite topic, to share news, theories, ideas, and to connect with others. The content from your Forum has been converted to Discussions posts, so nothing has been lost.

To learn more about what you can do here, check out https://community.fandom.com/wiki/Help:Discussions

If you're an admin on this community, read more about how you can customize your Discussions and set up guidelines for contributors: https://community.fandom.com/wiki/Help:Admin_and_Moderator_Tools_in_Discussions

Have fun!
",
);

$messages['qqq'] = array(
	'discussions-staff-welcome-title' => 'Title for the welcome post',
	'discussions-staff-welcome-post' => 'Message posted by a staff account when Discussions is first enabled for a community',
);

$messages['de'] = array(
	'discussions-staff-welcome-title' => 'Willkommen bei den Diskussionen!',
	'discussions-staff-welcome-post' => '
Hier ist der richtige Ort, um über deine Lieblingsthemen zu sprechen, um mit anderen Mitgliedern Neuigkeiten, Theorien und Ideen auszutauschen und dich mit ihnen zu vernetzen. Die Inhalte deines Forums wurden in Diskussionsbeiträge umgewandelt, es ist also nichts verloren gegangen.

Unter http://de.community.wikia.com/wiki/Hilfe:Diskussionen erfährst du mehr darüber, was du hier alles machen kannst.

Wenn du ein Admin dieser Community bist, kannst du unter http://de.community.wikia.com/wiki/Hilfe:Diskussionen#Moderation nachlesen, wie du deine Diskussionen moderieren, individuell anpassen und Richtlinien festsetzen kannst.

Viel Spaß!
',
);

$messages['es'] = array(
	'discussions-staff-welcome-title' => '¡Bienvenido a Discusiones!',
	'discussions-staff-welcome-post' => '
¡Bienvenido a Discusiones!

Este es el lugar para conversar acerca de tu tema favorito, compartir noticias, teorías, ideas, y para conectarse con otras personas como tú. El contenido que se encontraba en el Foro fue convertido en discusiones, así que nada se ha perdido.

Para aprender más acerca de lo que se puede hacer aquí, revisa http://comunidad.wikia.com/wiki/Ayuda:Discusiones

Si eres administrador de esta comunidad, aprende más acerca de como personalizar tus Discusiones y como configurar las guías comunitarias: http://comunidad.wikia.com/wiki/Ayuda:Herramientas_para_Administradores_y_Moderadores_en_discusiones

¡Diviértete!
',
);

$messages['fr'] = array(
	'discussions-staff-welcome-title' => 'Bienvenue dans Discussions !',
	'discussions-staff-welcome-post' => "
Vous pouvez ici parler de votre sujet favori, partager des actualités, des théories et des idées et aussi rencontrer d'autres personnes. Le contenu de votre Forum a été converti en publications sur Discussions, donc rien n'a été perdu.

Pour en savoir plus sur les possibilités offertes par cette fonctionnalité, accédez à la page http://communaute.wikia.com/wiki/Aide:Discussions.

Si vous êtes administrateur de cette communauté, vous pouvez personnaliser Discussions et définir des règles de contribution : http://communaute.wikia.com/wiki/Aide:Modération_des_Discussions.

Bonne continuation !
",
);

$messages['iris-4770'] = array(
	'discussions-staff-welcome-title' => '歡迎來到討論版！',
	'discussions-staff-welcome-post' => '您可以在這裡暢談您所感興趣的話題，分享訊息、理念、觀點，並且可以和其他人互動。論壇內容已經被移至討論版，您仍然可以查閱。

如果您希望進一步瞭解在這裡可以進行哪些活動，請查看： http://zh.community.wikia.com/wiki/Help:Discussions

如果您是這個社區的管理員，請閱讀如何自訂討論版以及如何為貢獻者制定方針：http://zh.community.wikia.com/wiki/Help:%E8%AE%A8%E8%AE%BA%E7%89%88#.E7.AE.A1.E7.90.86

祝您編輯愉快！',
);

$messages['iris-4850'] = array(
	'discussions-staff-welcome-title' => '歡迎來到討論版！',
	'discussions-staff-welcome-post' => '您可以在這裡暢談您所感興趣的話題，分享訊息、理念、觀點，並且可以和其他人互動。論壇內容已經被移至討論版，您仍然可以查閱。

如果您希望進一步瞭解在這裡可以進行哪些活動，請查看： http://zh.community.wikia.com/wiki/Help:Discussions

如果您是這個社區的管理員，請閱讀如何自訂討論版以及如何為貢獻者制定方針：http://zh.community.wikia.com/wiki/Help:%E8%AE%A8%E8%AE%BA%E7%89%88#.E7.AE.A1.E7.90.86

祝您編輯愉快！',
);

$messages['it'] = array(
	'discussions-staff-welcome-title' => 'Benvenuto in Discussioni!',
	'discussions-staff-welcome-post' => "
Questo è il luogo dove puoi parlare del tuo argomento preferito, condividere novità, teorie, idee e connetterti con gli altri. I contenuti del tuo Forum sono stati convertiti in post di Discussioni, pertanto nulla è andato perduto.

Per saperne di più su cosa puoi fare qui, dai un'occhiata a http://it.community.wikia.com/wiki/Aiuto:Discussioni

Se sei amministratore su questa community, leggi come puoi personalizzare Discussioni e come stabilire delle linee guida per gli utenti: http://it.community.wikia.com/wiki/Aiuto:Strumenti_di_moderazione_per_Discussioni

Buon divertimento!
",
);

$messages['ja'] = array(
	'discussions-staff-welcome-title' => 'ディスカッションへようこそ！',
	'discussions-staff-welcome-post' => '
ディスカッションへようこそ！ディスカッションはコミュニティ・メンバーが気になる話題について話したり、最新ニュースやアイディアなどを共有する場所です。フォーラムのコンテンツはディスカッションの投稿に変換されており、何も失われていませんのでご安心ください。

ディスカッションの楽しみ方の詳細はこちらをご覧ください：http://ja.community.wikia.com/wiki/ヘルプ:ディスカッション

本コミュニティのアドミンの方は、こちらからディスカッションのカスタマイズ方法や投稿者のガイドライン設定を確認できます：http://ja.community.wikia.com/wiki/Help:Admin_and_Moderator_Tools_in_Discussions
ぜひディスカッションをお楽しみください！
',
);

$messages['ko'] = array(
	'discussions-staff-welcome-post' => "
This is the place to talk about your favorite topic, to share news, theories, ideas, and to connect with others. The content from your Forum has been converted to Discussions posts, so nothing has been lost.

To learn more about what you can do here, check out https://community.fandom.com/wiki/Help:Discussions

If you're an admin on this community, read more about how you can customize your Discussions and set up guidelines for contributors: https://community.fandom.com/wiki/Help:Admin_and_Moderator_Tools_in_Discussions

Have fun!
",
	'discussions-staff-welcome-title' => 'Welcome to Discussions!',
);

$messages['nl'] = array(
	'discussions-staff-welcome-post' => "
This is the place to talk about your favorite topic, to share news, theories, ideas, and to connect with others. The content from your Forum has been converted to Discussions posts, so nothing has been lost.

To learn more about what you can do here, check out https://community.fandom.com/wiki/Help:Discussions

If you're an admin on this community, read more about how you can customize your Discussions and set up guidelines for contributors: https://community.fandom.com/wiki/Help:Admin_and_Moderator_Tools_in_Discussions

Have fun!
",
	'discussions-staff-welcome-title' => 'Welcome to Discussions!',
);

$messages['pl'] = array(
	'discussions-staff-welcome-title' => 'Witaj w Dyskusjach!',
	'discussions-staff-welcome-post' => '
Dyskusje to miejsce, w którym możesz rozmawiać na swój ulubiony temat, dzielić się wiadomościami oraz pomysłami, snuć teorie i poznawać innych fanów. Zawartość twojego Forum została przekształcona w posty Dyskusji, więc nic nie zginęło.

Żeby dowiedzieć się, co jeszcze możesz tutaj znaleźć, zobacz http://spolecznosc.wikia.com/wiki/Pomoc:Dyskusje.

Jeżeli jesteś administratorem tej społeczności, poznaj szczegóły jak dostosować stronę Dyskusji i dodać zasady dla innych współtwórców: http://spolecznosc.wikia.com/wiki/Pomoc:Narz%C4%99dzia_do_moderowania_Dyskusji

Miłej zabawy!
',
);

$messages['pt'] = array(
	'discussions-staff-welcome-title' => 'Bem-vindo a Discussões!',
	'discussions-staff-welcome-post' => '
Este é o lugar para falar sobre seu tema favorito, para compartilhar notícias, teorias, ideias e conectar-se outras pessoas. O conteúdo do seu Fórum foi convertido em posts de discussões, portanto nada foi perdido.

Para aprender mais sobre o que fazer aqui, confira 
http://comunidade.wikia.com/wiki/Ajuda:Discussões

Se você é um administrador nesta comunidade, leia mais sobre como você pode personalizar suas discussões e definir diretrizes para os colaboradores: 
http://comunidade.wikia.com/wiki/Ajuda:Ferramentas_de_administradores_e_moderadores_nas_Discussões

Divirta-se!
',
);

$messages['ru'] = array(
	'discussions-staff-welcome-title' => 'Добро пожаловать в Обсуждения!',
	'discussions-staff-welcome-post' => '
Здесь можно обсуждать любимые темы, делиться новостями, мыслями и идеями, а также общаться с другими участниками. Все записи Форума сохранились: мы просто перенесли их в Обсуждения.

Посмотрите, что еще вы можете делать в Обсуждениях: http://ru.community.wikia.com/wiki/Справка:Обсуждения

Если вы администратор этого сообщества, рекомендуем почитать о модерации Обсуждений и создании правил для участников: http://ru.community.wikia.com/wiki/Справка:Обсуждения/инструменты_администратора

Удачи!
',
);

$messages['vi'] = array(
	'discussions-staff-welcome-post' => "
This is the place to talk about your favorite topic, to share news, theories, ideas, and to connect with others. The content from your Forum has been converted to Discussions posts, so nothing has been lost.

To learn more about what you can do here, check out https://community.fandom.com/wiki/Help:Discussions

If you're an admin on this community, read more about how you can customize your Discussions and set up guidelines for contributors: https://community.fandom.com/wiki/Help:Admin_and_Moderator_Tools_in_Discussions

Have fun!
",
	'discussions-staff-welcome-title' => 'Welcome to Discussions!',
);

$messages['zh-hans'] = array(
	'discussions-staff-welcome-title' => '欢迎来到讨论版！',
	'discussions-staff-welcome-post' => '您可以在这里畅谈您所感兴趣的话题，分享新闻、理念、观点，并且可以和其他人互动。论坛内容已经被移至讨论版，您可以继续查阅。

如果您希望了解在这里可以进行哪些活动，请查看：http://zh.community.wikia.com/wiki/Help:讨论版

如果您是这个社区的管理员，请阅读如何对讨论版进行定制化以及如何为贡献者制定社区方针：http://zh.community.wikia.com/wiki/Help:讨论版#.E7.AE.A1.E7.90.86

祝您编辑愉快！',
);

$messages['zh-hant'] = array(
	'discussions-staff-welcome-title' => '歡迎來到討論版！',
	'discussions-staff-welcome-post' => '您可以在這裡暢談您所感興趣的話題，分享訊息、理念、觀點，並且可以和其他人互動。論壇內容已經被移至討論版，您仍然可以查閱。

如果您希望進一步瞭解在這裡可以進行哪些活動，請查看： http://zh.community.wikia.com/wiki/Help:Discussions

如果您是這個社區的管理員，請閱讀如何自訂討論版以及如何為貢獻者制定方針：http://zh.community.wikia.com/wiki/Help:%E8%AE%A8%E8%AE%BA%E7%89%88#.E7.AE.A1.E7.90.86

祝您編輯愉快！',
);

