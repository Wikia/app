...
Manager.prototype.handleClient = function (data, req) {
  ...
  if(data.transport &&  data.id && this.handshaken &&   this.handshaken[data.id]) {
	  this.handshaken[data.id]['transport'] = data.transport;	  
	 require('util').log("CON:" +  data.transport );
  }
...
Manager.prototype.garbageCollection = function () {
  ...
  while (i--) {
    ...
    if((now - handshake.timefull) >= 1000*60*60*5 ) {
	 require('util').log("DCON:" +  handshake.transport );
    	this.onClientDisconnect(ids[i]);
    }
