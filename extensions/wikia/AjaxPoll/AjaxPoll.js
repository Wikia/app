...
YAHOO.AjaxPoll.Callback = {
	success: function( response ) {
		 ...
		 */
		var bars = YAHOO.util.Dom.getElementsByClassName( "wpPollBar" + answer["id"], "span" );
		for( var b in bars ) {
			/**
			...
			if( bar_id.match( /^wpPollBar\w{32}\-(\d+)$/ ) ) {
				var key = RegExp.$1;
				if( votes[ key ] ) {
					YAHOO.util.Dom.setStyle( [ bar_id ], "width", votes[ key ][ "pixels" ] + "px" );
					YAHOO.util.Dom.get( "wpPollVote" + answer["id"] + "-" + key ).innerHTML = votes[ key ][ "value" ];
					YAHOO.util.Dom.get( "wpPollPercent" + answer["id"] + "-" + key ).innerHTML = votes[ key ][ "percent" ];
				}
				else {
					YAHOO.util.Dom.setStyle( [ bar_id ], "width", "0px" );
					YAHOO.util.Dom.get( "wpPollVote" + answer["id"] + "-" + key ).innerHTML = 0;
					YAHOO.util.Dom.get( "wpPollPercent" + answer["id"] + "-" + key ).innerHTML = 0;
				}
			}
		}
