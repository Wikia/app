...
class GetQuestionWidget extends UnlistedSpecialPage {
	...
	{
		global $wgOut, $wgScriptPath, $wgServer, $wgSitename;
			...
			<script type=\"text/javascript\">
					function updatePreview(){
						...
						if( document.getElementById('askbox').checked ){
							ask_box = \"<input type='text' value='" . wfMsg("ask_a_question") . "' id='wikia_ask' style='width:\" + (document.getElementById('widget_width').value - 50) + \"px' onfocus=this.value='' > <input type='button' value='" . wfMsg("ask_button") . "' onclick=javascript:wikia_ask_question()><scr\" + \"ipt>function wikia_ask_question(){window.location=\'" . $wgServer . $wgScriptPath . "/index.php?title=Special:CreateQuestionPage&questiontitle=' + document.getElementById('wikia_ask').value + '&categories=\" + ask_category + \"'}</scr\" + \"ipt>\";
						}
						preview = \"<scr\" + \"ipt>wikia_answers_background_color = '\" + document.getElementById('backgroundcolor').value + \"';wikia_answers_width = '\" + document.getElementById('widget_width').value + \"';wikia_answers_border = '1px solid #000000';wikia_answers_link_color = '\" + document.getElementById('linkcolor').value + \"';</scr\" + \"ipt><scr\" + \"ipt type='text/javascript' src='" . $wgServer . $wgScriptPath . "/index.php?action=ajax&rs=wfGetQuestionsWidget&rsargs[]=\" + document.getElementById('widget_title').value + \"&rsargs[]=\" + category + \"&rsargs[]=\" + document.getElementById('number').value + \"&rsargs[]=\" + document.getElementById('order').value + \"'></scri\" + \"pt><noscript><a href='" . $wgServer . "'>" . $wgSitename . "</a></noscript>\";
	...
	myAutoComp.generateRequest = function(sQuery) {
		return \"" . $wgServer . $wgScriptPath . "/index.php?action=ajax&rs=wfGetCategoriesSuggest&rsargs[]=\" + sQuery + \"&rsargs[]=5\"
	};
