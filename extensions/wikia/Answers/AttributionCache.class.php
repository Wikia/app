...
class AttributionCache {
	...
	private function rebuildArticleContribs( Title $title ) {
		...
		$res =
			$dbs->select( 'revision', [ 'rev_user', 'rev_user_text' ],
				[ 'rev_page' => $title->getArticleID() ], __METHOD__,
				[ 'GROUP BY' => 'rev_user' ] );
		...
		while ( $row = $dbs->fetchObject( $res ) ) {
			if ( !in_array( $row->rev_user, $wgAnswerHelperIDs ) ) {
				...
				if ( $row->rev_user == $firstRevisionUserId ) {
					$pageUserEditPoints =
						$this->rebuildUserEditPoints( $row->rev_user, $title->getArticleID() );
				...
				$results[] =
					$this->getContribsEntry( $row->rev_user,
	...
	public function getFirstRevisionUserId( Title $title ) {
		$dbs = wfGetDB( DB_MASTER );
		$res =
			$dbs->selectRow( 'revision', array( 'rev_user' ),
	...
	private function sortContribs( Array $contribs ) {
		...
		foreach ( $tmpSorted as $userId => $editsNum ) {
			$sortedContribs[] =
				$this->getContribsEntry( $userId, $editsNum, $tmpUserNames[$userId] );
		}
	...
	private function rebuildUserEditPoints( $userId, $pageId = 0 ) {
		...
		$edits =
			$dbs->selectRow( $tables, array( 'min(rev_timestamp) AS date, count(rev_id) AS count' ),
	...
	public function updateArticleContribs( Title $title, User $user ) {
		...
		if ( in_array( $user->getId(), $wgAnswerHelperIDs ) ) {
			...
			if ( !empty( $editPoints ) ) {
				$editPoints ++;
				$this->setUserEditPointsToCache( $userId, $editPoints,
		...
		if ( !empty( $contribs ) ) {
			...
			foreach ( $contribs as &$contribData ) {
				if ( $contribData['user_id'] == $userId ) {
					$contribData['edits'] =
						( $userId == 0 ) ? ( $contribData['edits'] + 1 ) : $editPoints;
					$userExists = true;
					break;
				}
			}
			...
			if ( !$userExists ) {
				...
				$newEntry =
					$this->getContribsEntry( $userId, ( ( $userId == 0 ) ? 1 : $editPoints ),
						$user->getName() );
	...
	function getFirstContributor( $page_title_id ) {
		...
		if ( empty( $data ) ) {
			...
			$params['LIMIT'] = 1;
			$s =
				$dbr->selectRow( 'revision', array( 'rev_user', 'rev_user_text' ),
					array( 'rev_page' => $page_title_id ), __METHOD__, $params );
			...
			$num1 =
				$this->getContribsEntry( $s->rev_user, $editsNum,
