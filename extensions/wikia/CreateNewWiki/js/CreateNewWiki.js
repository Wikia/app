var WikiBuilder = {
	...
	init: function() {
		...
		$('#NameWiki input.next').click(function() {
			if (!that.wikiDomain.val() || !that.wikiName.val() || $('#NameWiki .wiki-name-error').html() || $('#NameWiki .wiki-domain-error').html() || that.nameAjax || that.domainAjax) {
				that.nameWikiSubmitError.show().html(WikiBuilderCfg['name-wiki-submit-error']).delay(3000).fadeOut();
			} else {
				that.saveState({
