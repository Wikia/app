<section id="CustomizeTab" class="CustomizeTab">
	<fieldset class="background">
		<h1><?= wfMessage( 'themedesigner-background' )->text() ?></h1>
		<ul>
			<li class="wrap-color">
			...
			<? if ( $wg->OasisResponsive ) : ?>
				<li class="wrap-middle-color">
					<h2><?= wfMessage( 'themedesigner-color-middle' )->text() ?></h2>
			...
			<li class="wrap-background">
				...
				<label for="tile-background">
					...
					<?= wfMessage( 'themedesigner-tile-background' )->text() ?>
				</label>
				<label for="fix-background">
					<input type="checkbox" id="fix-background">
					<?= wfMessage( 'themedesigner-fix-background' )->text() ?>
				</label>
				<? //TODO: Remove this after global release of responsive layout  ?>
				<? if ( $wg->OasisResponsive ) : ?>
					<span class="not-split-option">
						<label for="not-split-background">
							...
							<?= wfMessage( 'themedesigner-not-split-background' )->text() ?>
						</label>
	...
	<fieldset class="page">
		...
		<ul>
			...
			<li>
				<h2><?= wfMessage( 'themedesigner-links' )->text() ?></h2>
				<img src="<?= $wg->BlankImgUrl ?>" class="color-links" id="swatch-color-links">
			</li>
			<li>
				<h2><?= wfMessage( 'themedesigner-header' )->text() ?></h2>
			...
			<li>
				...
				<h2><?= wfMessage( 'themedesigner-transparency' )->text() ?></h2>
