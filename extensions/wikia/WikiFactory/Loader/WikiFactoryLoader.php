...
class WikiFactoryLoader {
	...
	public function execute() {
		...
		if( ! isset( $this->mVariables["wgDBname"] ) ) {
			...
			// todo don't set these in CommonSettings
			if ( !empty( $this->langCode ) ) {
				foreach ( [ 'wgArticlePath', 'wgUploadPath', 'wgScript', 'wgLogo', 'wgScriptPath' ] as $varName ) {
					$this->mVariables[$varName] = '/' . $this->langCode . ( $this->mVariables[$varName] ?? '' );
				}
			}
