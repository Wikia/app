require(['mw', 'jquery', 'wikia.window'], function(mw, $, window) {
	mw.loader.using('jquery.autocomplete').then(function () {
		...
		var $searchInputWrapper = $('#searchInputWrapperTest');
		var searchSuggestionsUrl = $searchInput.data('suggestions-url');
		...
		console.error('failed to attach search suggestions to local nav search (experiment)');
	});
});