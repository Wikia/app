...
class DPLMain
{
	...
	*/
    ...
    public static function dynamicPageList( $input, $params, &$parser, &$bReset, $calledInMode ) {
		...
		$sCount = $wgRequest->getVal('DPL_count','');
		$iCount = -1;
    ...
    // ###### PARSE PARAMETERS ######
        ...
        foreach($aParams as $iParam => $sParam) {		
            ...
            if( count( $aParam ) < 2 ) {
                if (trim($aParam[0])!='') $output .= $logger->escapeMsg(DPL_i18n::WARN_UNKNOWNPARAM, $aParam[0]. " [missing '=']", self::validParametersList());
                continue;
            }
            $sType = trim($aParam[0]);
            $sArg = trim($aParam[1]);
            ...
            if( $sType=='') {
                $output .= $logger->escapeMsg(DPL_i18n::WARN_UNKNOWNPARAM, '[empty string]', self::validParametersList());
                continue;
            }
            ...
            switch ($sType) {
                ...
                case 'namespace':
                    ...
                    foreach($aParams as $sParam) {
						...
						} else if ( in_array($sNs, array_keys(ExtDynamicPageList::$options['namespace'])) ) {
	                        $aNamespaces[] = $sNs;
							$bSelectionCriteriaFound=true;
						} else {
            ...
            if (ExtDynamicPageList::$functionalRichness <= 0) {
				$output .= $logger->escapeMsg(DPL_i18n::WARN_UNKNOWNPARAM, $sType, self::validParametersList());
				continue;
			}
    ...
    // ###### BUILD SQL QUERY ######
		...
		if ( !ExtDynamicPageList::$allowUnlimitedResults 	&& $sGoal != 'categories' 
															&& strpos($sResultsHeader.$sResultsFooter,'%TOTALPAGES%')!==false) $sSqlCalcFoundRows = 'SQL_CALC_FOUND_ROWS';
    ...
    // ###### PROCESS SQL QUERY ######
        ...
        if ($dbr->numRows( $res ) <= 0) {
            ...
            if ($sNoResultsHeader == '' && $sNoResultsFooter == '')	$output .= $logger->escapeMsg(DPL_i18n::WARN_NORESULTS);
            $dbr->freeResult( $res );
            return $output;
        }
