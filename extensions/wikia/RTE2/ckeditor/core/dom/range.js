...
( function() {
	...
	function getCheckStartEndBlockEvalFunction() {
		...
		return function( node ) {
			...
			//Wikia - start
			if ( node.$.nodeType == CKEDITOR.NODE.COMMENT ) {
				return false;
			}
			//Wikia - end
					...
					while ( enlargeable || sibling ) {
						...
						while ( sibling ) {
							...
							if ( sibling.type == CKEDITOR.NODE_COMMENT ) {
								...
								if ( sibling.$.nodeType == CKEDITOR.NODE_ELEMENT &&  ( sibling.$.offsetWidth > offsetWidth0 || excludeBrs && sibling.is( 'br' ) ) && !sibling.data( 'cke-bookmark' ) ) {
