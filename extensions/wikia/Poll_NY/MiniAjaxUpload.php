...
$wgExtensionFunctions[] = 'wfSpecialPollAjaxUpload';
...
function wfSpecialPollAjaxUpload(){
	...
	SpecialPage::addPage( new PollAjaxUpload );
...
class PollAjaxUploadForm extends UploadForm{
		...
		$wgOut->addHTML( "
			...
			<form id='upload' name='upload'  onSubmit=\"return submitForm();\" method='post' enctype='multipart/form-data' action=\"$action\">
				<table border='0' cellpadding='0' cellspacing='0' class='upload-form'>		
					...
					<tr>
						<td>
							<input tabindex='9' type='submit' id='wpUpload' name='wpUpload' value=\"" . wfMsgForContent( 'upload' ) . "\" />
						</td>
					</tr>
				</table>
			</form>
	...
	function showSuccess() {
		...
		$dbr->insert( '`imagelinks`',
		array(
			'il_from' => 0,
			'il_to' => $this->mDestName
			), __METHOD__
		);
