...
class TagCloud {
	...
	public function __construct($limit=10, $sQueryString='') {
		$this->limit = $limit;
	...
	public function initialize(){
		...
		$sql = $this->query ? $this->query : "SELECT replace(replace(cl_to,'_News',''),'_Opinions','') as cl_to, count(*) as count FROM {$dbr->tableName( 'categorylinks' )} cl1
			GROUP BY cl_to
			ORDER BY
			count DESC
			LIMIT 0,{$this->limit}";
