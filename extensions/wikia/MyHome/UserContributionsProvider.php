	...
	public function get($limit = 10) {
		...
		foreach ($res['query']['usercontribs'] as &$entry) {
			...
			$result[++$i] = array(
				...
				'diff' => empty($entry['rev_parent_id']) ? '' : $title->getLocalURL('diff=' . $entry['revid'] . '&oldid=' . $entry['rev_parent_id'])
			);

			if (MWNamespace::isTalk($entry['ns']) || in_array($entry['ns'], array(400 /* video */, NS_USER, NS_TEMPLATE, NS_MEDIAWIKI))) {
				$result[$i]['title'] = $title->getPrefixedText();
			}

			if (defined('NS_BLOG_ARTICLE_TALK') && $entry['ns'] == NS_BLOG_ARTICLE_TALK) {
				$tmp = explode('/', $result[$i]['title']);
				$result[$i]['title'] = wfMsg('myhome-namespace-blog') . ':' . $tmp[1];
			}
