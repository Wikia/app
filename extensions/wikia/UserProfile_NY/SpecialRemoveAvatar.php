...
class RemoveAvatar extends SpecialPage {
	...
	function execute( $user ){
		...
		$wgSpecialPages['RemoveAvatar'] = array('SpecialPage','RemoveAvatar','avatarremove');
		...
		if( $wgRequest->wasPosted() ) {
			...
			$wgOut->addHTML( "<div>" . wfMsg("avatarupload_removesuccess") . "</div>" );
			$wgOut->addHTML( "<div><a href=\"" . $this->title->escapeFullURL() . "\">" . wfMsg("avatarupload_removeanother") . "</a></div>" );
		}else{
			if( $user ){
	...
	function showUserAvatar( $user_name ){
		...
		$output = "";
		$output .= "<div><b>" . wfMsg("avatarupload_currentavatar", $user_name). "</b></div><p>";
	...
	function deleteImage( $id, $size ){
		...
		if( $img ){
			...
			if( $wgEnableUploadInfoExt ) {
				...
				UploadInfo::log( $user->getUserPage(), $wgUploadDirectory . "/avatars/" .  $img, "", "r" );
			}
		}
