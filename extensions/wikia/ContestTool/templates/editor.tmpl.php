...
<div id="PanePreview"<?= empty($formData['messagePreview']) ? ' style="display:none"' : '' ?>>
	<fieldset>
	<legend><?= wfMsg('cntool-label-preview') ?></legend>
...
<div id="PaneCompose">
		...
		<form method="post" id="msgForm" action="<?= $title->getLocalUrl() ?>">
			<fieldset>
				<legend><?= wfMsg('cntool-label-content') ?></legend>
			...
			<div id="PaneButtons">
				<input name="coPreview" type="submit" value="<?= wfMsg('cntool-button-preview') ?>" id="fPreview"/>
				<input name="coSave" type="submit" value="<?= wfMsg('cntool-button-save') ?>" id="fSave"/>
			</div>
		</form>
</div>
<!-- e:<?= __FILE__ ?> -->
