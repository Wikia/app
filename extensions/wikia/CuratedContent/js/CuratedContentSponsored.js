...
$(function () {
	...
	require( ['wikia.window', 'jquery', 'wikia.nirvana', 'JSMessages'], function ( window, $, nirvana, msg ) {
		...
		var d = document,
			...
			requiredError = msg( 'wikiaCuratedContent-sponsored-required-entry' ),
			emptyTagError = msg( 'wikiaCuratedContent-sponsored-empty-tag' ),
			orphanedVideo = msg( 'wikiaCuratedContent-sponsored-orphaned-video' ),
			sure = msg( 'wikiaCuratedContent-sponsored-delete-videos-are-you-sure' ),
			videoDoesNotExist = msg( 'wikiaCuratedContent-sponsored-video-does-not-exist' ),
			videoIsNotOoyala = msg( 'wikiaCuratedContent-sponsored-video-is-not-ooyala' ),
		...
		$save.on( 'click', function () {
			...
			if ( checkForm() ) {
				...
				nirvana.sendRequest({
					controller: 'CuratedContentSpecialSponsored',
					method: 'save',
					data: {
						languages: data
					}
