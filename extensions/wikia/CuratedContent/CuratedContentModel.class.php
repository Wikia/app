...
class CuratedContentModel{
	...
	public function getWikiContents(){
		...
		if ( empty( $ret ) ) {
			...
			$ret[ 'searchURL' ] = $searchTitle->getLocalUrl( array( 'useskin' => CuratedContentController::SKIN_NAME ) );
			$ret[ 'entries' ] = Array();

			$entries = array_filter(
				explode( "\n", strip_tags( str_replace( array( '<br>', '<br/>', '<br />' ), "\n" , wfMsgForContent( 'wikiaCuratedContent-contents' ) ) ) ),
		...
		if ( $category ) {
			...
			if ( empty( $contents ) ) {
				...
				foreach( $titles as $title ) {
					$contents[] = array(
						'title' => $title->getText(),
						'url' => $title->getLocalUrl( array( 'useskin' => CuratedContentController::SKIN_NAME ) )
					);
				}
	...
	public function getSearchResults( $term, $totalLimit = self::SEARCH_RESULTS_LIMIT ){
		...
		if ( !empty( $this->app->wg->EnableWikiaSearchExt ) && !empty( $term ) ) {
			wfloadExtensionMessages( 'CuratedContent' );
					...
					foreach ( $resultSet as $result ) {
						try {
							$textResults[] = array(
									...
									'urlForm' => $mwService->getLocalUrlForPageId( $result['pageid'], array( 'useskin' => CuratedContentController::SKIN_NAME ) )
									);
							$count++;
