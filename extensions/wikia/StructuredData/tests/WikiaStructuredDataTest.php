...
class WikiaStructuredDataTest extends WikiaBaseTest {
	...
	private function getPropertyByLabel($array, $schemaName) {
		foreach($array['properties'] as $v) {
			if ($v['label'] == $schemaName) return $v;
		}
		return null;
	}
	/**
	...
	public function testSDElement() {
		...
		$a = $sdElement->toArray();
		$description = $this->getPropertyByLabel($a, 'description' );

		$this->assertEquals( $description['type'], 'rdf:Literal' );
		$this->assertEquals( $description['name'], 'schema:description' );
	}
}