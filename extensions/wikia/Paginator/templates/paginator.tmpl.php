	...
	<?	$i = 1;
		...
		?>
			<? if ( ( $i === 1) && ( $page < $currentPage ) ){
				?><li><a href="<?=str_replace( '%s', ( $currentPage - 1 ), $url); ?>" data-back="true" data-page="<?=$currentPage - 1; ?>" class="button secondary paginator-prev"><span>Previous</span></a></li><?;
			} ?>
			<? if ( $page === '' ){
			...
			<? if ( ( $i === count($pages)) && ( $page > $currentPage ) ){
				?><li><a href="<?=str_replace( '%s', ( $currentPage + 1 ), $url); ?>" data-page="<?=$currentPage + 1; ?>" class="button secondary paginator-next"><span>Next</span></a></li><?;
			} ?>
		<?
		$i++;
		} ?>
