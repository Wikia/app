...
class WikiaBarModel extends WikiaBarModelBase {
	...
	private function convertURLs( $data ) {
		global $wgWikiaBaseDomain;

		foreach ( $data as $key => $val ) {
			if ( substr( $key, -4 ) === 'href'
				&& strpos( $val, $wgWikiaBaseDomain ) !== false
				&& wfHttpsAllowedForURL( $val )
			) {
				$data[$key] = wfProtocolUrlToRelative( $val );
			}
		}
