...
describe('toc module', function () {
	...
	var sections = {
		...
		window = {
			document: {
				body: getBody('<div id=wkTOC class=side-menu><header>TOCHEADER</header>' +
				'<img id=wkTOCHandle src=""></div>'),
		...
		mustache = {
			render: function () {
				return '<ol class="toc-list level"><li class="has-children first-children">' +
				'<a href="#Biography">Biography<span class="chevron right"></span></a><ol class="toc-list level1">' +
				'<li class="has-children"><a href="#Season_One">Season One</a><ol class="toc-list level2">' +
				'<li><a href="#Pilot">Pilot</a></li></ol></ol></ol>';
			}
		},
		wikiaToc = {
			getData: function () {
	...
	it('should init itself', function () {
		...
		expect(jQuery.fn.on.calls.argsFor(0)[0]).toEqual('curtain:hidden');
		expect(jQuery.fn.on.calls.argsFor(0)[1]).toEqual(jasmine.any(Function));

		expect(jQuery.fn.on.calls.argsFor(1)[0]).toEqual('click');
		expect(jQuery.fn.on.calls.argsFor(1)[1]).toEqual(jasmine.any(Function));
	});
	...
	it('should track events', function () {
		...
		expect(track.event).toHaveBeenCalledWith('newtoc', undefined, {
			label: 'open'
		});
		expect(track.event).toHaveBeenCalledWith('newtoc', undefined, {
			label: 'close'
