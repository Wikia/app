$(function () {
	require( ['wikia.window', 'wikia.cookies', 'wikia.loader', 'JSMessages'], function ( window, cookie, loader, msg ) {
		var ua = window.navigator.userAgent, standalone = navigator.standalone, // Check if it's already a standalone web app or running within a webui view of an app (not mobile safari)
			type;
				...
				resources: {
					...
					require( ['smartbanner'], function ( sb ) {
						sb({
							...
							icon: window.wgExtensionsPath + window.wgAppIcon// The URL of the icon (defaults to <meta name="apple-touch-icon">)
						});
					});
				});
		}
