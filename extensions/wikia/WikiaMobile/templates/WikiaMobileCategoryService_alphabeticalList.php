...
<section class="alphaSec noWrap">
	<header>
		<?= $wf->Message( 'wikiamobile-categories-items-total', $wg->ContLang->formatNum( $total ) )->inContentLanguage()->text() ;?>
		<button class=wkBtn id=expAll>
			<span class=expand><?= $wf->Message( 'wikiamobile-categories-expand' )->inContentLanguage()->text() ;?></span>
...
<? foreach ( $collections as $index => $collection) {
	...
	<section id=<?= $id ;?> class=artSec data-batches=<?= $itemsBatch['batches'] ;?>>
		<a class="pagLess<?= ( $currentBatch > 1 ? ' visible' : '' ) ;?>" data-batch="<?=$prevBatch?>" href="?page=<?=$prevBatch;?>&index=<?=$urlSafeIndex;?>#<?=$id;?>"><?= $wf->Message( 'wikiamobile-category-items-prev' )->text() ;?></a>
		<?= $app->getView( 'WikiaMobileCategoryService', 'getBatch', array( 'itemsBatch' => $itemsBatch['items'] ) ) ;?>
		<a class="pagMore<?= ( $itemsBatch['next']  ? ' visible' : '' ) ;?>" data-batch="<?=$nextBatch;?>" href="?page=<?=$nextBatch;?>&index=<?=$urlSafeIndex;?>#<?=$id;?>"><?= $wf->Message( 'wikiamobile-category-items-more' )->text() ;?></a>
	</section>
<? } ?>
</section>