 ...
 * @group UsingDB
 */
class WikiaMapsUploadImageFromFileTest extends WikiaBaseTest {
	...
	public function testVerifyUpload_success_not_for_POI() {
		...
		/**
		 * @var WikiaMapsUploadImageFromFile $uploadImageFromFileMock
		 */
		$this->assertEquals(
			$expected,
	...
	public function testVerifyUpload_failed_for_POI() {
		$expected = [ 'status' => WikiaMapsUploadImageFromFile::POI_CATEGORY_MARKER_IMAGE_TOO_SMALL_ERROR ];
		$uploadImageFromFileMock = $this->getUploadImageFromFileMock(
			[ 'status' => 'success' ],
			true,
			true,
	...
	private function getUploadImageFromFileMock(
		...
		$uploadImageFromFileMock = $this->getMock( 'WikiaMapsUploadImageFromFile', [
			'getUploadDetails',
			'isUploadSuccessful',
			'isUploadPoiCategory',
