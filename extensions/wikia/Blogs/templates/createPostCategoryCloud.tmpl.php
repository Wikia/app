...
<div id="categoryCloudDiv" style="display: block;">
	...
	<?php if (!empty($cloud->tags)) : ?>
		<?php $xnum = 0; foreach( $cloud->tags as $xname => $xtag): ?>
			...
			<a href="#" id="cloud<?=$cloudNo;?>_<?=$xnum?>" onclick="cloudToggle(<?=$cloudNo;?>, '#wpCategoryTextarea<?=$cloudNo;?>', escape ('<?=$xname?>'), <?=$xnum?>); return false;"><?=$xname?></a>
			</span>
		<?php $xnum++; endforeach; ?>
	<?php endif ?>	
	</div>
...
/*<![CDATA[*/
	$(document).ready( function () {
		CreateBlogPost.CheckCategoryCloud(<?=$cloudNo;?>, '#wpCategoryTextarea<?=$cloudNo;?>', '#categoryCloudSection<?=$cloudNo;?>');
	});
/*]]>*/
</script>
<!-- e:<?= __FILE__ ?> -->
