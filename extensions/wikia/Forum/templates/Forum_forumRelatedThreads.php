	...
	<section class="module WikiaActivityModule ForumActivityModule">
		...
		<ul>
			...
			<li>
				...
				<div class="edited-by">
					<?php
					$user = '<a href="' . Sanitizer::encodeAttribute( $messageLast->getUser()->getUserPage()->getFullUrl() ) . '">' . htmlspecialchars( $messageLast->getUser()->getName() ) . '</a>';
					$time = '<span class="timeago abstimeago" title="' . $messageLast->getCreateTime() . '" alt="' . $messageLast->getCreateTime( TS_MW ) . '">&nbsp;</span>'
					?>
