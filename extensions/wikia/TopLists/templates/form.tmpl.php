...
<form id="toplist-editor" name="toplist<?= $mode ;?>" method="POST" action="">
	...
	<div class="InputSet">
		...
		<? if ( $mode == 'create' ) :?>
			<input type="text" name="list_name" placeholder="<?= wfMsg( 'toplists-editor-title-placeholder' ) ;?>" value="<?= $listName ;?>"<?= ( !empty( $errors[ 'list_name' ] ) ) ? ' class="error"' : null ;?> />
	...
	<div class="InputSet AutoCompleteWrapper">
		...
		<input type="text" name="related_article_name" placeholder="<?= wfMsg( 'toplists-editor-related-article-placeholder' ) ;?>" autocomplete="off" value="<?= $relatedArticleName ;?>"<?= ( !empty( $errors[ 'related_article_name' ] ) ) ? ' class="error"' : null ;?> />
