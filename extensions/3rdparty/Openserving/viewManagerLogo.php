<?php
$wgExtensionFunctions[] = 'wfSpecialViewManagerLogo';


function wfSpecialViewManagerLogo(){
	global $wgUser,$IP;
	include_once("includes/SpecialPage.php");
	
	class ViewManagerLogo extends SpecialPage {
...
class ViewLogo {
	...
	var $fileExtensions;
	//$avatarUploadDirectory = $wgUploadDirectory . "/Avatars"; 
	/**#@-*/
	 ...
	 */
	function ViewLogo( &$request ) {
		global $wgUser, $wgSiteView;
		
		//$this->mDestFile          = $wgUser->mId;
		$this->fileExtensions = array(   'gif',  'jpg', 'jepg', 'png' );
		$this->viewName      = $_GET["name"];
	...
	function execute() {
		...
		$this->avatarUploadDirectory = $wgUploadDirectory . "/views"; 
	...
	function getLogo($name){
		$files = glob(dirname( __FILE__ ) . "/../images/views/" . $name . "*");
	...
	function processUpload() {
		...
		$this->mUploadSaveName = $this->viewName . "_logo." . $finalExt;
		 ...
		 */
		 ...
		 //clear old files
			 if(is_file($this->avatarUploadDirectory . "/" . $this->viewName . "_logo.jpg") ) unlink($this->avatarUploadDirectory . "/" . $this->viewName . "_logo.jpg");
			 if(is_file($this->avatarUploadDirectory . "/" . $this->viewName . "_logo.gif") ) unlink($this->avatarUploadDirectory . "/" . $this->viewName . "_logo.gif");
			 if(is_file($this->avatarUploadDirectory . "/" . $this->viewName . "_logo.png") ) unlink($this->avatarUploadDirectory . "/" . $this->viewName . "_logo.png");
		//end
	...
	function showSuccess($status) {
		...
		$ext = "jpg";
		$wgOut->addScript("<script type=\"text/javascript\" src=\"extensions/avatar.js\"></script>\n");
		/*
		$output = '<div id="ava">
        <div id="ava_img">
            <div id="ava_overlay"></div>
            <div id="ava_drager"></div>
            <img src=images/avatars/' . $wgUser->mId . '_l.' . $ext . ' id="avatar" />
        </div>

        <div id="ava_slider">

            <div id="ava_handle"></div>
        </div>
        ...
        <input type="hidden" id="ava_image" name="file" value="userimages/david.pean/u_orig.jpg" />
        <input type="hidden" name="username" value="david.pean">
        <input type="hidden" name="process" value="1">
		<script>setupAva()</script>
    </div>';
	*/
	
		$output = "<h2>Upload Success</h2>";
		$output .= "<h5>Below is your new logo</h5>";
		
		
		$output .= "<table cellspacing=0 cellpadding=5>";
		$output .= "<tr><td><img src=images/views/" . $this->getLogo($this->viewName)  . "?r=" . rand() . "></td></tr>";
	...
	" ;
		...
		if($this->getLogo($this->viewName ) != ""){
			$output = "<table><tr><td style='color:#666666;font-weight:800'>Current Image</td></tr>";
			$output .= "<tr><td><img src=images/views/" . $this->getLogo($this->viewName ) . "></td></tr></table><br>";
		}
		$wgOut->addHTML($output);
	...
	<input tabindex='1' type='file' name='wpUploadFile' id='wpUploadFile' " . ($this->mDestFile?"":"onchange='fillDestFilename()' ") . " style='width:100px' />
	</td></tr><tr>
	{$source}
	</tr>
	<tr><td>
