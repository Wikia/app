...
$wgExtensionFunctions[] = 'wfSetupMobileSearches';
...
function wfSetupMobileSearches(){
	global $IP;
...
function wfMobileSearches(){
	...
	if(isset($_POST['artist']) && isset($_POST['song'])){
		...
		if(($songResult['lyrics'] == $failedLyrics) || ($songResult['lyrics'] == "")){
			...
			if(mysql_query("DELETE FROM $TABLE_NAME WHERE request_artist='$artist' AND request_song='$song'", $db)){
				print "Deleted.";
			} else {
