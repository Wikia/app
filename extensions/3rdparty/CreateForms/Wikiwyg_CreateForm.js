...
proto.config.controlLabels.wikify = 'Wikify';
...
proto.format_img = function(element) {
    ...
    if(uri.indexOf("YouTube_placeholder.gif") > -1){
		...
		this.assert_space_or_newline();
			...
			wiki_img = "[[Image:" + img_name + ((img_width)?"|" + img_width:"") + "]]";
...
Wikiwyg.Wysiwyg.prototype.enableThis = function() {
    ...
    this.edit_iframe.style.border = '1px #dcdcdc solid';
	...
	//this.get_edit_document().body.fontSize = '16px';
...
Wikiwyg.Wysiwyg.prototype.do_youtube = function() {
    ...
    $("insert_link").onclick = function() {
		...
		self.insert_youtube($F("wikiwyg_link_url") )
		Element.remove(LinkWindow.editWindow)
    };
}
...
Wikiwyg.Wysiwyg.prototype.do_link = function() {
    ...
    $("insert_link").onclick = function() {
		...
		self.insert_link($F("wikiwyg_link_url"),$F("wikiwyg_link_text"));
		Element.remove(LinkWindow.editWindow)
    };
}
