...
class SpecialAbuseLog extends SpecialPage {
	...
	function showList() {
		...
		if (!$skipQuery) {
			...
			if ( $this->mSearchFilter
				&& ( !AbuseFilter::filterHidden( $this->mSearchFilter ) || AbuseFilterView::canViewPrivate() )
			) {
	...
	function formatRow( $row, $li = true ) {
		...
		$username = User::getUsername( $row->afl_user, $row->afl_user_text );
		if ( !$row->afl_wiki ) {
			// Local user
			$userLink = $sk->userLink( $row->afl_user, $username ) .
				$sk->userToolLinks( $row->afl_user, $username );
		} else {
			$userLink = WikiMap::foreignUserLink( $row->afl_wiki, $username );
