...
function addImageTags() {
    if ( gid('canEditPage') == null ) { /* page isn't editable */
        ...
        var subdomainRe = /(.*).wikia.com/;
        matches = subdomainRe.exec(domain);
...
function submitTag() {
    ...
    if ( request ) {
        ...
        var args = "rect="+getStringTagRect()+"&imgName="+escape(imgValue)+"&tagName="+encodeURIComponent(tagValue)+"&wpEditToken="+gid('wpEditToken').value;
...
function removeTag(tagID, elem, tagValue) {
    ...
    if ( request ) {
        ...
        var args = "rect="+getStringTagRect()+"&tagID="+tagID+"&imgName="+escape(gid('imgName').value)+"&tagName="+encodeURIComponent(tagValue);
        request.open("GET", url + "&" + args, true);
        request.onload = function(e) {
            if ( request && request.responseText ) {
