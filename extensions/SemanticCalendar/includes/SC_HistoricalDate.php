...
class SCHistoricalDate {
	...
	static protected function leap_jul_greg($year) {
		return (($year < 1582) ? SCHistoricalDate::leap_julian($year) : SCHistoricalDate::leap_gregorian($year));
	}
	...
	protected function createFromGregorian($year, $month, $day) {
		$this->m_date = (self::GREGORIAN_EPOCH - 1) +
			...
			(($month <= 2) ? 0 :
				(SCHistoricalDate::leap_gregorian($year) ? -1 : -2)
			) + $day);
	}
	...
	static function daysInMonth($year, $month) {
		if ($month == 4 || $month == 6 || $month == 9 || $month == 11)
			return 30;
		if ($month == 2)
			return (SCHistoricalDate::leap_jul_greg($year)) ? 29 : 28;
		return 31;
	}
