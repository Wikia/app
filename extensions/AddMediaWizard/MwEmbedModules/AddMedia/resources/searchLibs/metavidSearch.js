	...
	getProviderResults: function( search_query, callback ) {
		...
		$.getJSON( url + '&cb=?&cb_inx=1', request, function( data ) {
			...
			for ( var i in _this.resultsObj ) {
				...
				var proe = new mw.Uri( resource['roe_url'] );
				resource['start_time'] = proe.query['t'].split( '/' )[0];
				resource['end_time'] = proe.query['t'].split( '/' )[1];
				resource['stream_name'] = proe.query['stream_name'];
