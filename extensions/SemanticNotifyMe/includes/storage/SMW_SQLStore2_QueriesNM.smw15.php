...
//		}
	 ...
	 */
	// modified by ning, based on getDebugQueryResult, getInstanceQueryResult
	protected function getNMQueryResult( SMWQuery $query, $rootid ) {
		...
		$res = $this->m_dbs->select( $this->m_dbs->tableName( $qobj->jointable ) . " AS $qobj->alias" . $qobj->from, "DISTINCT $qobj->alias.smw_title AS t,$qobj->alias.smw_namespace AS ns", $qobj->where, 'SMW::getNMQueryResult' );
...
//							   " AS $subquery->alias $subquery->from" . ( $subquery->where ? " WHERE $subquery->where":'' );
						$sql[] = "(SELECT $subquery->joinfield AS id FROM " . $this->m_dbs->tableName( $subquery->jointable ) .
								" AS $subquery->alias $subquery->from" . ( $subquery->where ? " WHERE $subquery->where":'' ) . ")";
