/**
 * This file handles fallback for link elements with data URIs generated by
 * this extension.  Generally it will only affect IE6/7.  However, we do not
 * sniff for browser name or version, so other non-compliant browsers should be
 * handled as well.
 *
 * @file
 * @ingroup Extensions
...
(function( $ ) {
	$.fn.cssExtensionDataURIFallback = function( id, color ) {
		// Install the canary and set its inital color
		var canary = $( '<div>' )
			.attr( 'id', id )
			.css( 'display', 'none' )
		...
		if ( style.get( 0 ).styleSheet ) {
			// IE6/7 (our main fallback targets) only work this way.
			style.get( 0 ).styleSheet.cssText = text;
		} else {
			style.text( text );
		}
		style.appendTo( 'head' );
	 ...
	 * @license http://www.webtoolkit.info/license
	 */
	var Base64 = {
		// private property
