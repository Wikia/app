...
class File_Ogg_Theora extends File_Ogg_Media
{
    ...
    function getKeyFrameIndex(){
    	$kAry = array();
    	foreach($this->_streamList as & $packet){
    		# Reject -1 as a granule pos, that means no segment finished in the packet
    		if ( $packet['abs_granule_pos'] != 'ffffffffffffffff' ) {
				//get the top & bottom word:
	    		$topWord = floatval( base_convert( substr( $packet['abs_granule_pos'], 0, 8 ), 16, 10 ) );
	        	$bottomWord = floatval( base_convert( substr( $packet['abs_granule_pos'], 8, 8 ), 16, 10 ) );
	        	...
	        	$keyFramePos = $topWord / pow(2, $this->_kfgShift - 32) +
	           	...
	           	$kTime = ( $keyFramePos ) / $this->_frameRate;
				$time =  ( $keyFramePos + $offset ) / $this->_frameRate;
	    		/*print "paket:: " . print_r($packet, true) . "
	   				K:$kTime
	   				rt:$time ( " . ( $offset / $this->_frameRate ) . " \n\n";*/
				if(!isset($kAry[ (string)$kTime ])){
	   				$kAry[ (string)$kTime ] = $packet['head_offset'];
				}
    		}
    	}
    	return $kAry;
