 ...
 * the HTML for the input field to be placed in the form.
 *
 ...
 *	                         label-message
 *	'tip'                 -- Additional hint to be displayed above the label
 *	                         Hidden by default and shown when hovering [?] (tipsy).
 *	                         Always shown if javascript is disabled.
 ...
 *                           Overwrites 'help'.
 *  'help'                -- alternatively, raw help text. Overriden by 'help-message'
 *                           and help-messages'.
...
class HTMLStyleForm {
	...
	function wrapForm( $html ) {
		...
		$attribs = array(
			...
			'method'  => $this->mMethod,
			'class'   => 'mw-htmlform-form visualClear',
			'enctype' => $encType,
		);
		if ( !empty( $this->mId ) ) {
			$attribs['id'] = $this->mId;
		...
		foreach ( $fields as $key => $value ) {
			if ( is_object( $value ) ) {
				$v = empty( $value->mParams['nodata'] )
					? $this->mFieldData[$key]
					: $value->getDefault();
				$formInnerHtml .= $value->getFormRow( $v );
...
abstract class HTMLStyleFormField {
	 ...
	 * form elements, <div> tags, or labels/help messages.
	...
	function __construct( $params ) {
		...
		if ( isset( $params['label-message'] ) ) {
			...
			$this->mLabel = wfMsgExt( $msg, 'parseinline', $msgInfo ) . wfMsg( 'colon-separator' );
...
class HTMLStyleInfoField extends HTMLStyleFormField {
	...
	function getFormRow( $value ) {
		...
		return parent::getFormRow( $value );
	}
...
class HTMLStyleHiddenField extends HTMLStyleFormField {
	...
	public function getFormRow( $value ) {
		$params = array();
		if ( $this->mID ) {
			$params['id'] = $this->mID;
		}
...
class HTMLStyleEditTools extends HTMLStyleFormField {
	...
	public function getFormRow( $value ) {
		...
		return '<div class="mw-htmlform-field-HTMLEditTools"><div class="mw-input">'
			. '<div class="mw-editTools">' . $msg->parseAsBlock() . "</div>"
			. "</div></div>\n";
	}
}
