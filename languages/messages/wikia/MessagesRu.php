<?php

#Related to visitor skins: Slate.php and Smoke.php
$messages = array_merge( $messages, array(
'login_greeting' => "Добро пожаловать в Wikia, [[User:$1|$1]]!",
'create_an_account' => "Зарегистрироваться",
'login_as_another' => "Войти под другим именем",
'not_you' => "Не Вы?",
'this_wiki' => "Эта wiki",
'home' => "Домой",
'forum' => "Форум",
'helpfaq' => "Помощь и FAQ",
'createpage' => "Создать новую страницу",
'joinnow' => "Регистрируйтесь прямо сейчас",
'most_popular_articles' => "Самые популярные статьи",
'this_article' => "Эта статья",
'this_page' => "Эта страница",
'edit_contribute' => "Редактировать / Дополнить",
'discuss' => "Обсудить",
'share_it' => "Поделиться:",
'my_stuff' => "Моё",
'choose_reason' => "Выберите причину",
'top_five' => "Пятерка лучших",
'most_popular' => "Самые популярные",
'most_visited' => "Самые посещаемые",
'newly_changed' => "Недавно обновленные",
'highest_ratings' => "Самые высокие рейтинги",
'most_emailed' => "Самые популярные по количеству электронных писем",
'rate_it' => "Оцените:",
'unrate_it' => "Сбросить оценку",
'use_old_formatting' => 'Используйте старый формат',
'use_new_formatting' => 'Neue Formatierung verwenden',
'review_reason_1' => 'рассмотрите рассуждают 1',
'review_reason_2' => 'рассмотрите рассуждают 2',
'review_reason_3' => 'рассмотрите рассуждают 3',
'review_reason_4' => 'рассмотрите рассуждают 4',
'review_reason_5' => 'рассмотрите рассуждают 5',
'copyrightpage' => 'w:Wikia:Licensing|Wikia:Licensing',
'addnewtalksection-link' => 'Создать новый раздел',

'tog-enableuserjs' => 'Включить персональный JavaScript ([[Справка:Персональный СSS и JavaScript|подробнее]])',
'wikia-pagetitle' => '$1 — Wikia',

'recentchanges-on-wikia-title' => 'Свежие правки на этой вики',
'recentchanges-on-wikia-content' => "Представляем вам классификацию шаблонов — отсортируйте шаблоны на этой вики по типам, чтобы сделать [[Special:Templates|работу с шаблонами]] проще, и улучшить отображение разных шаблонов на мобильных устройствах. [[Справка:Классификация шаблонов|Узнайте больше о типах шаблонов]], или '''[[Special:Insights/templateswithouttype|начните работу прямо сейчас]]'''!

Для [[Справка:Инфобоксы|мультиплатформенных инфобоксов]] теперь доступны табберы для изображений и сворачиваемые элементы.

Как смотрится ваш контент на разных устройствах? Узнай об этом на [[w:c:portability|Portability.wikia.com]].",
) );
