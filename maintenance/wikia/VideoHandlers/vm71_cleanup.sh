...
echo "select distinct wiki_id, wiki_id, wiki_dbname from video_migration_log where action_name='POSTMIGRATION' and action_status='STOP'" | /usr/wikia/backend/bin/slave dataware > $TMPFILE
cat $TMPFILE
