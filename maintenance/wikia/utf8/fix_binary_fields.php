...
// config - begin
$binaryColumns = array(
	'math' => array( 'math_inputhash', 'math_outputhash' ),
);
// config - end
...
//				$sqlList[] = "ALTER TABLE `{$table->TABLE_NAME}` DEFAULT CHARACTER SET utf8 COLLATE utf8_bin;";
	...
	protected function processFields() {
		global $binaryColumns;
		$fields = array();
		foreach ($binaryColumns as $table => $fieldNames) {
			$tableColumns = $this->walker->getFieldsFor($table);
			if (!$tableColumns) continue;
			foreach ($tableColumns as $column) {
				if ( array_search($column->COLUMN_NAME,$fieldNames) !== false )
					$fields[] = $column;
			}
		}
...
//						$sqlList[] = $tableDump;
				...
				if ($transColumnType) {
					...
					$this->script->table($field->TABLE_NAME)->add("MODIFY {$field->COLUMN_NAME} {$field->COLUMN_TYPE} BINARY {$rest}");
				} else {
