...
class Utf8md4DbConvert {
	...
	public function __construct( $options ) {
		...
		$this->walker = new ConvertionDatabaseWalker( $this->db );

		$this->script = new SqlScriptUtil();
	}
	...
	protected function executeScript() {
		$mode = SqlScriptUtil::MODE_DEFAULT;
		if ( $this->quick ) {
			$mode = SqlScriptUtil::MODE_QUICK;
		}
 ...
 * used options:
 */
$maintenance = new Utf8md4DbConvert( $options );
$maintenance->execute();
