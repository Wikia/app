...
class LyricsWikiCrawlerTest extends WikiaBaseTest {
	...
	/**
	 * @dataProvider addIfValidDataProvider
	 */
	public function testAddIfValid( $message, $inputs, $expected ) {
		$results = [];

		foreach( $inputs as $input ) {
			$this->crawler->addIfValid( $results, $input );
		}

		$this->assertEquals( $expected, $results, $message );
	}

	public function addIfValidDataProvider() {
		return [
			[
				'empty inputs',
				'inputs' => [],
