...
class FixWikiMainImage extends Maintenance {
	...
	protected function getWikisToFix() {
		...
		$result = $dbr->select(
			...
			array( '*' ),
			array(
			    'city_main_image' => null,
			    'city_lang_code' => F::app()->wg->ContLang->getCode() 
			),
			__METHOD__
		);
	...
	protected function updateVisualizationMainImageName($wikiId, $imageName) {
		...
		$db->update(
		    CityVisualization::CITY_VISUALIZATION_TABLE_NAME, 
		    ['city_main_image' => $imageName], 
		    ['city_id' => $wikiId, 'city_lang_code' => F::app()->wg->ContLang->getCode()], 
		    __METHOD__
		);
		$db->commit();
	}
