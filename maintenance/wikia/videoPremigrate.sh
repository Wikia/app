...
group by wiki_id order by count(*) desc
limit 5;" | slave stats > $TMPFILE
cat $TMPFILE
cat $TMPFILE | while read line; do
	...
	SERVER_ID=$cityid php videoPremigrate.php --conf /usr/wikia/docroot/wiki.factory/LocalSettings.php
	echo "\n\n"
done
rm -f $TMPFILE