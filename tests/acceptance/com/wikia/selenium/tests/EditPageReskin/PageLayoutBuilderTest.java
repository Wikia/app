...
public class PageLayoutBuilderTest extends EditPageBaseTest {

	private void openPageLayoutBuilder() throws Exception {
		loginAsStaff();
	...
	public void testPageLayoutBuilderSpecialPageLayout() throws Exception {
		String layoutTitle = "LayoutTest" + Integer.toString(new Random().nextInt(9999));

		this.openPageLayoutBuilder();
		...
		assertTrue(session().isElementPresent("//header[@id='EditPageHeader']//h1/a[contains(text(), '" + layoutTitle + "foo')]"));
		...
		// header should be updated with new title
		assertTrue(session().isElementPresent("//header[@id='EditPageHeader']//h1/a[contains(text(), '" + layoutTitle + "')]"));
		...
		assertTrue(session().isElementPresent("//div[@id='EditPageToolbar']/div[contains(@class, 'cke_toolbar_widescreen')]"));
